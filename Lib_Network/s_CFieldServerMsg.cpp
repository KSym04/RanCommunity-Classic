	...
	MSG_LIST* pMsg )
{
	...
	case NET_MSG_FIELD_REQ_JOIN:
		{
			GLMSG::SNETJOIN2FIELDSERVER* pNNgj = reinterpret_cast<GLMSG::SNETJOIN2FIELDSERVER*> (pMsg->Buffer);
			DWORD dwClientID = pNNgj->m_dwClientID;
			if ( dwClientID > 0 )  MsgFieldReqJoin(pMsg);
			else CConsoleMessage::GetInstance()->Write(_T("ERROR: Reject Field REQ join (%d)"), dwClientID);
		}
			break;
...
void CFieldServer::MsgJoinInfoFromClient(MSG_LIST* pMsg)
{
	...
	// 캐릭터 번호 세팅
	m_pClientManager->SetChaNum(dwClient, pGLChar->GetCharID());
...
void CFieldServer::MsgGameJoinChar ( MSG_LIST* pMsg )
{
	...
	m_pClientManager->SetClientValid ( dwClient );
	m_pClientManager->SetClientCheck ( dwClient );
...
void CFieldServer::MsgFieldReqJoin ( MSG_LIST* pMsg )
{
    ...
    if (pMsg == NULL)
    {
        CConsoleMessage::GetInstance()->Write(_T("ERROR: pMsg is null"));
        return;
    }
	...
	if (pNgj == NULL)
	{
        CConsoleMessage::GetInstance()->Write(_T("ERROR: pNgj is null"));
        return;
    }
    ...
    // 
    if (!pCHAR_DATA.get()) {
        CConsoleMessage::GetInstance()->Write(_T("ERROR: pCHAR_DATA is null"));
        return;
    }
	...
	int userID = m_pClientManager->GetUserNum(dwClientID);
    if (userID <= 0 || userID > 4294967290 ) {
        CConsoleMessage::GetInstance()->Write(_T("ERROR: INVALID_USER_ID for client %d"), dwClientID);
        return;
    }
	 DWORD dwUserID = static_cast<DWORD>(userID);  // 
	//	케릭터의 사용자 ID 유효성 검사.
    //
    if (dwUserID != static_cast<DWORD>(pCHAR_DATA->GetUserID())) {
        CConsoleMessage::GetInstance()->Write(
            _T("ERROR: Character UserID and Message UserID are unmatched %d : %d"),
            dwUserID, pCHAR_DATA->GetUserID()
        );
        return;
    }
...
void CFieldServer::MsgFieldReqJoin( DWORD dwClientID, SCHARDATA2 * pCHAR_DATA, GLMSG::SNETJOIN2FIELDSERVER::SINFO & sINFO )
{
	...
	if (pCHAR_DATA->m_dwCharID == 0) 
	{
        CConsoleMessage::GetInstance()->Write(_T("ERROR: pCHAR_DATA is not initialized correctly."));
        return;
    }
	...
	if ( dwUserID != (DWORD) pCHAR_DATA->GetUserID() )
	{
		...
		//CConsoleMessage::GetInstance()->Write(_T("ERROR:Character UserID and Message UserID are unmatched")); // TEST LANG ROEL
		 CConsoleMessage::GetInstance()->Write(_T("ERROR: UserID mismatch. Expected: %d, Got: %d"), dwUserID, pCHAR_DATA->GetUserID());
		return;
	}
...
//	char strTemp[100];
	...
	while (token != NULL)
	{
		...
		if ( strcmp(token, "c") == 0 )
		{
			CConsoleMessage::GetInstance()->Write("%d %d/%d",(int) GLGaeaServer::GetInstance().GetNumPC(),
			(int) m_pClientManager->GetCurrentClientNumber(),
			(int) m_pClientManager->GetMaxClient());
			return 0;
		}

		if (strcmp(token, _T("af_t")) == 0)
		{
			token = strtok(NULL, seps);
			if (token)
			{
				SenseiBLOCKER()->SetAntiFloodTime(atoi(token));
				CConsoleMessage::GetInstance()->Write("AntiFloodTime = %d", SenseiBLOCKER()->GetAntiFloodTime());
		...
		if (strcmp(token, "af_on") == 0)
		{
			SenseiBLOCKER()->SetAntiFlood(TRUE);
			if (SenseiBLOCKER()->IsAntiFlood())
				CConsoleMessage::GetInstance()->Write("AntiFlood ON");
			return 0;
		}

		if (strcmp(token, "af_off") == 0)
		{
			SenseiBLOCKER()->SetAntiFlood(FALSE);
			if (!SenseiBLOCKER()->IsAntiFlood())
					CConsoleMessage::GetInstance()->Write("AntiFlood OFF");

			return 0;
		}

		if (strcmp(token, _T("checker")) == 0)
		{
			token = strtok(NULL, seps);
			if (token)
			{
				SenseiBLOCKER()->SetClientCheckerTimer(atoi(token));
				CConsoleMessage::GetInstance()->Write("ClientCheckTime = %d", SenseiBLOCKER()->GetClientCheckerTime());
		...
		if (strcmp(token, "check_on") == 0)
		{
			SenseiBLOCKER()->SetClientChecker(TRUE);
			if (SenseiBLOCKER()->IsClientChecker())
				CConsoleMessage::GetInstance()->Write("ClientChecker ON");
			return 0;
		}

		if (strcmp(token, "check_off") == 0)
		{
			SenseiBLOCKER()->SetClientChecker(FALSE);
			if (!SenseiBLOCKER()->IsClientChecker())
					CConsoleMessage::GetInstance()->Write("ClientChecker OFF");
			return 0;
		}
		// 서버 채팅 메시지, global
