...
CLoginServer::CLoginServer(HWND hWnd, HWND hEditBox, HWND hEditBoxInfo, HWND hEditBoxStats)
	: CServer(hWnd, hEditBox, hEditBoxInfo, hEditBoxStats), m_pClientManager(NULL), m_dwLastGameSvrUpdateTime(0), m_bInspecting(false)
{
...
bool CLoginServer::CheckAllowIP(SOCKET socket)
{
	...
	if (!CCfg::GetInstance()->GetAllowFileLoad())
		return TRUE;
	if (!GetAllowIP())
		return TRUE;
...
int CLoginServer::WorkProc()
{
	...
	while (m_bIsRunning)
	{
		...
		case NET_RECV_POSTED:
		{
			...
			while (true)
			{
				pNmg = (NET_MSG_GENERIC *)m_pClientManager->getRcvMsg(dwClient, FALSE, bBlocked);
				if (NULL == pNmg)
				{
					break;
				}
...
int CLoginServer::ExecuteCommand(char *strCmd)
{
	...
	while (token != NULL)
	{
		...
		if (strcmp(token, "af_doblock") == 0)
		{
			SenseiBLOCKER()->DoBlocking();
			CConsoleMessage::GetInstance()->Write("AntiFlood -> Adding Lists to Firewall");
			return 0;
		}
