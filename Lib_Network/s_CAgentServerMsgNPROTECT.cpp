...
void CAgentServer::MsgGameGuardAnswer(MSG_LIST *pMsg)
{
	...
	if (_tcslen(szEncKey) > 0)
	{
		...
		// FIX (Eifelzocker): ensure full copy with parentheses and enforce null terminator.
		...
		{
			CConsoleMessage::GetInstance()->Write(_T("Error HeartBit (GameGuard) EncryptKey mismatch %s %s , closing client %d"), szEncryptKey, szEncKey, pMsg->dwClient);
			// Eifelzocker fix: actively close invalid session to prevent buildup.
