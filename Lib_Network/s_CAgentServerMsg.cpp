...
// Added 2025-08-21: track heartbeat encryption key mismatches (no C++11 dependency)
#include <windows.h>
static volatile LONG g_HeartbeatEncKeyMismatchCount = 0; // use InterlockedIncrement
...
int CAgentServer::MsgProcess(MSG_LIST *pMsg)
{
	...
	switch (nmg->nType)
	{
	case NET_MSG_CLIENT_CHECK_FB:
	{
		NET_CLIENT_CHECK_FB *nmg2 = (NET_CLIENT_CHECK_FB *)nmg;

		if (nmg2->nResult == m_pClientManager->GetClientResult(pMsg->dwClient))
		{
			m_pClientManager->SetClientCheck(pMsg->dwClient);
		}
		else
		{
			CConsoleMessage::GetInstance()->Write("ClientChecker ClosingIP(%s)", m_pClientManager->GetClientIP(pMsg->dwClient));
	...
	case NET_MSG_HEARTBEAT_CLIENT_ANS:
	{
		...
		if (_tcslen(szEncKey) > 0)
		{
			...
			// FIX (Eifelzocker): correct copy length (parentheses) and force null termination.
			memcpy(szEncryptKey, nmg2->szEnCrypt, sizeof(TCHAR) * (ENCRYPT_KEY + 1));
			szEncryptKey[ENCRYPT_KEY] = 0;
			m_Tea.decrypt(szEncryptKey, ENCRYPT_KEY + 1);
			if (_tcscmp(szEncryptKey, szEncKey))
			{
				LONG mismatch = InterlockedIncrement(&g_HeartbeatEncKeyMismatchCount);
				if ((mismatch % 50) == 0)
				{
					CConsoleMessage::GetInstance()->Write(_T("Heartbeat EncryptKey mismatch count=%ld (showing every 50) last:%s expected:%s UserID:%s Client:%d"),
														  mismatch, szEncryptKey, szEncKey, m_pClientManager->GetUserID(pMsg->dwClient), pMsg->dwClient);
				}
				// Eifelzocker fix: Close the invalid client to prevent accumulation of bad sessions.
				CloseClient(pMsg->dwClient);
				break;
			}
		}
	...
	// Taiwan / Hongkong
	case NET_MSG_LOGIN_2:
		MsgLogIn(pMsg);
		break;
	// China
	case CHINA_NET_MSG_LOGIN:
		ChinaMsgLogin(pMsg);
		break;
	// Thailand 로그인 요청
	case THAI_NET_MSG_LOGIN:
		ThaiMsgLogin(pMsg);
		break;
	case DAUM_NET_MSG_LOGIN:
		DaumMsgLogin(pMsg);
		break;
	case TERRA_NET_MSG_LOGIN:
		TerraMsgLogin(pMsg);
		break;
	case GSP_NET_MSG_LOGIN:
		GspMsgLogin(pMsg);
		break;
	case EXCITE_NET_MSG_LOGIN:
		ExciteMsgLogin(pMsg);
		break;
	// 일본 서비스사 이전(Gonzo)에 따른 추가
	case JAPAN_NET_MSG_LOGIN:
		JapanMsgLogin(pMsg);
		break;
	case GS_NET_MSG_LOGIN:
		GsMsgLogin(pMsg);
		break;

	// Daum 2차 패스워드 체크
	case DAUM_NET_MSG_PASSCHECK:
		DaumMsgPassCheck(pMsg);
		break;
	// 말레이지아 2차 패스워드 체크
	case TERRA_NET_MSG_PASSCHECK:
		TerraMsgPassCheck(pMsg);
		break;

	// 일본 2차 패스워츠 체크
	case EXCITE_NET_MSG_PASSCHECK:
		ExciteMsgPassCheck(pMsg);
		break;

	// Client->Agent : 새로운 캐릭터 생성
	case NET_MSG_CHA_NEW:
		MsgCreateCharacter(pMsg);
		break;
	case NET_MSG_REQ_RAND_KEY:
		MsgSndRandomNumber(dwClient);
		break;
	// Client->Agent : 사용자 (디스플레이용)  캐릭터 기초정보 요청
	case NET_MSG_REQ_CHA_BAINFO:
		MsgSndChaBasicBAInfo(pMsg);
		break;
	// Client->Agent : 특정캐릭의 기초정보 요청 (로비 디스플레이용)
	case NET_MSG_REQ_CHA_BINFO:
		MsgSndChaBasicInfo(pMsg);
		break;
	// Client->Agent : 캐릭터 삭제
	case NET_MSG_CHA_DEL:
		MsgSndChaDelInfo(pMsg);
		break;
	case DAUM_NET_MSG_CHA_DEL:
		DaumMsgSndChaDelInfo(pMsg);
		break;
	case TERRA_NET_MSG_CHA_DEL:
		TerraMsgSndChaDelInfo(pMsg);
		break;
	case EXCITE_NET_MSG_CHA_DEL:
		ExciteMsgSndChaDelInfo(pMsg);
		break;
	case GSP_NET_MSG_CHA_DEL:
		GspMsgSndChaDelInfo(pMsg);
		break;
	case JAPAN_NET_MSG_CHA_DEL:
		JapanMsgSndChaDelInfo(pMsg);
		break;
	case GS_NET_MSG_CHA_DEL:
		GsMsgSndChaDelInfo(pMsg);
		break;

	// Client->Agent : 캐릭터 선택후 조인
	case NET_MSG_LOBBY_GAME_JOIN:
		MsgGameJoin(pMsg);
		break;
	// DB 스레드->에이젼트서버 : DB에서 읽어진 정보로 케릭터 접속 시도.
	case NET_MSG_AGENT_REQ_JOIN:
		MsgAgentReqJoin(pMsg);
		break;
	// Field->Agent : 캐릭터가 조인했음
	case MET_MSG_GAME_JOIN_FIELDSVR_FB:
		MsgLobbyCharJoinField(pMsg);
		break;
	...
	case NET_MSG_APEX_DATA:
		...
		break;
	// Apex 적용( 홍콩 )
	case NET_MSG_APEX_RETURN:
		MsgApexReturn(pMsg);
		break;

	// GameGuard 인증용 메세지
	case NET_MSG_GAMEGUARD_ANSWER:
		MsgGameGuardAnswer(pMsg);
		break;
	// GameGuard 인증용 메세지 #1
	case NET_MSG_GAMEGUARD_ANSWER_1:
		MsgGameGuardAnswerFirst(pMsg);
		break;
	// GameGuard 인증용 메세지 #2
	case NET_MSG_GAMEGUARD_ANSWER_2:
		MsgGameGuardAnswerSecond(pMsg);
		break;
	// 필드서버와 클라이언트에서 받은 추적 로그를 찍는다.
	case NET_MSG_LOG_UPDATE_TRACING_CHARACTER:
		MsgTracingLogPrint(pMsg);
		break;

	/*register page, Juver, 2017/11/18 */
	case NET_MSG_REGISTER_INIT:
		RegisterInit(pMsg);
		break;
	case NET_MSG_REGISTER_ACTION:
		RegisterAction(pMsg);
		break;

	/* user account page, Juver, 2020/11/05 */
	case NET_MSG_USER_ACCOUNT_PAGE_INIT:
		UserAccountPageInit(pMsg);
		break;
	...
	/*dmk14 ingame web*/
	case NET_MSG_FORGOTPASS:
		MsgForgotPass(pMsg);
		break;
	case NET_MSG_CHANGEPASS:
		MsgForgotChangePass(pMsg);
		break;
	case NET_MSG_SELCHAR_CHANGEPASS:
		MsgSelcharChangePass(pMsg);
		break;
	case NET_MSG_SELCHAR_CHANGEPIN:
		MsgSelcharChangePin(pMsg);
		break;
	case NET_MSG_REQ_GAMETIME_CONVERT:
		MsgReqGameTimeConvert(pMsg);
		break;
	case NET_MSG_SELCHAR_RESETPIN:
		MsgSelcharResetPin(pMsg);
		break;
	case NET_MSG_SELCHAR_TOPUP:
		MsgSelcharTopUp(pMsg);
		break;
	case NET_MSG_SELCHAR_CHANGEMAIL:
		MsgSelcharChangeMail(pMsg);
		break;
	...
	case NET_MSG_GCTRL_CAPTURE_THE_FLAG_C2A_SCORE_INFO_REQ:
	{
		...
		if (dwMAX_CLIENT <= dwAgentClient)
			dwAgentClient -= dwMAX_CLIENT;
	...
	case NET_MSG_GM_WHERE_PC_MAP:
		MsgGmwherePcMap(nmg, dwClient, dwGaeaID);
		break;
	case NET_MSG_SERVER_PLAYERKILLING_MODE:
		MsgGmPkMode(nmg, dwClient, dwGaeaID);
		break;

	case NET_MSG_VIETNAM_TIME_RESET:
		MsgVietTimeReset(nmg, dwClient, dwGaeaID);
		break; // vietnamtest%%% && vietnamtest2
...
void CAgentServer::OtherMsgProcess(MSG_LIST *pMsg)
{
	...
	case NET_MSG_GCTRL_REQ_GLOBAL_RANKING_LEVEL_UPDATE: /*Top Level*/
...
void CAgentServer::MsgAgentReqJoin(MSG_LIST *pMsg)
{
	...
	if (!m_pClientManager->IsAccountPass(dwClientID))
		return;
...
int CAgentServer::MsgLobbyCharJoinField(MSG_LIST *pMsg)
{
	...
	// Check GaeaID
	if (pMsgJoinFB->dwGaeaID != m_pClientManager->GetGaeaID(dwClient))
		return NET_ERROR;
