...
/*Top Level*/
int COdbcManager::GetTopLevel(VEC_TOP_LEVEL& vecLevel)
{
	SQLRETURN sReturn	= 0;
	ODBC_STMT* pConn	= m_pGameDB->GetConnection();
	ODBC_STMT* pConn2	= m_pUserDB->GetConnection();
	...
	TCHAR szTemp[512] = { 0 };
	_snprintf(szTemp, 512, "SELECT DISTINCT TOP 50 "
		"P.ChaNum, P.ChaName, P.ChaLevel, P.ChaClass, P.ChaSchool, P.GuNum, P.ChaExp "
		"FROM RanUser.dbo.UserInfo U, RanGame1.dbo.ChaInfo P "
		"WHERE P.UserNum = U.UserNum AND U.UserType < 11 AND U.UserBlockDate < GETDATE() AND P.ChaDeleted != 1 AND P.ChaExp > 0 "
		"ORDER BY P.ChaExp DESC ");
	...
	while (true)
	{
		...
		if (sReturn == SQL_SUCCESS || sReturn == SQL_SUCCESS_WITH_INFO)
		{
			...
			::SQLGetData(pConn->hStmt, 5, SQL_C_LONG, &nChaSchool,	0,					&cbChaSchool);
			::SQLGetData(pConn->hStmt, 6, SQL_C_LONG, &nGuNum,		0,					&cbGuNum);
			::SQLGetData(pConn->hStmt, 7, SQL_C_LONG, &llChaExp,	0,					&cbChaExp);

			sTopLevel.dwChaNum = (DWORD)nChaNum;

			if (cbChaName != 0 && cbChaName != -1)
				::StringCchCopy(sTopLevel.szChaName, CHR_ID_LENGTH, (const char*)szChaName);

			sTopLevel.wChaLevel		= (WORD)nChaLevel;
			sTopLevel.nChaClass		= nChaClass;
			sTopLevel.wChaSchool		= (WORD)nChaSchool;
			sTopLevel.wGuNum			= (WORD)nGuNum;
			sTopLevel.llnExp		= llChaExp;

			vecLevel.push_back(sTopLevel);
		}
		else break;
