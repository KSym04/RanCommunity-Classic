...
// Direct include to guarantee CMinLZ4 definition visibility even if s_NetGlobal include order changes
#include "MinLZ4.h"
...
CServer::CServer(
	...
	HWND hEditBoxInfo,
	HWND hEditBoxStats)
	: m_hWnd(hWnd), m_hEditBox(hEditBox), m_hEditBoxInfo(hEditBoxInfo), m_hEditBoxStats(hEditBoxStats), m_hIOServer(NULL)

	  ,
	  m_hAcceptThread(NULL), m_hUpdateThread(NULL)

	  ,
	  m_hUpdateQuitEvent(INVALID_HANDLE_VALUE), m_dwUpdateWaitTime(0), m_dwUpdateOldTime(0), m_dwUpdateFrame(10)

	  ,
	  m_pRecvIOCP(NULL), m_pSendIOCP(NULL)

	  ,
	  m_pRecvMsgManager(NULL), m_sServer(INVALID_SOCKET) // Server Socket
	  ,
	  m_nStatus(S_SERVER_STOP)

	  ,
	  m_dwCompKey(0), m_nPort(0) // Server port
	  ,
	  ...
	  ,
	  m_dwLastDropClient(0), m_bIsRunning(false), m_bUpdateEnd(false), m_bTestServer(false), m_bAdult(false), m_bPK(true), m_bPKLess(false)
	  ...
	  ,
	  m_dwCpuCount(0), m_dwLangSet(0), m_dwHeartBeatTime(0), m_dwFrameTime(0), m_dwFrameCount(0), m_nServerType(0), bRPCIDHashValid(FALSE) /*ccfg security, Juver, 2018/07/08 */

	  ,
	  m_bAllowIP(false)

	  ,
	  m_pSenseiBLOCKER(NULL)
{
	...
	// Initialize LZ4 compression library (replaces legacy LZO)
	CMinLZ4::GetInstance().init();
	...
	m_hSenseiBLOCKER = LoadLibrary("SenseiBLOCKER.dll");
	if (!m_hSenseiBLOCKER)
	{
		char msg[256];
		sprintf(msg, "SenseiBLOCKER.dll Load Failed GetLastError() %d", GetLastError());
		MessageBox(NULL, msg, "LoadLibrary", NULL);
	}

	if (m_hSenseiBLOCKER)
	{
		SENSEI_INIT SenseiBLOCKER_INIT = (SENSEI_INIT)GetProcAddress(m_hSenseiBLOCKER, "getSensei");

		m_pSenseiBLOCKER = SenseiBLOCKER_INIT();

		if (!m_pSenseiBLOCKER)
			MessageBox(NULL, "[FAILED] SenseiBLOCKER_INIT", "Sensei", NULL);
		else
		{
			HRESULT hr = m_pSenseiBLOCKER->OneTimeInit();
			if (FAILED(hr))
				MessageBox(NULL, "[FAILED] OneTimeInit", "Sensei", NULL);
		}
	}
}
