...
CSessionServer::CSessionServer(HWND hWnd, HWND hEditBox, HWND hEditBoxInfo, HWND hEditBoxStats)
	: CServer(hWnd, hEditBox, hEditBoxInfo, hEditBoxStats), m_pClientManager(NULL)
{
	m_dwLoginServerID = -1;
...
void CSessionServer::CloseClient(DWORD dwClient)
{
		...
		m_pClientManager->GetSvrType(dwClient) == SERVER_LOGIN)
	{
		...
		for (DWORD dwL = NET_RESERVED_SLOT; dwL < static_cast<DWORD>(m_nMaxClient); dwL++)
		{
...
void CSessionServer::IPFilterCleanIP(std::string strIP)
{
	...
	for (int i = 0; i < m_pClientManager->GetMaxClient(); ++i)
	{
		if (!m_pClientManager->IsOnline(i))
			continue;
		if (m_pClientManager->GetSvrType(i) == SERVER_AGENT)
			continue;
		if (m_pClientManager->GetSvrType(i) == SERVER_FIELD)
			continue;
		if (m_pClientManager->GetSvrType(i) == SERVER_SESSION)
			continue;
		if (m_pClientManager->GetSvrType(i) == SERVER_LOGIN)
			continue;
...
void CSessionServer::MsgServerBroadCastIPFilterWhiteListAdd(MSG_LIST *_pMsg)
{
	...
	for (DWORD dwNum = 0; dwNum < (DWORD)m_nMaxClient; ++dwNum)
	{
		if (m_pClientManager->GetSvrType(dwNum) == SERVER_AGENT ||
			...
			m_pClientManager->GetSvrType(dwNum) == SERVER_LOGIN)
		{
			if (netMsg->nServerType == NET_SERVER_AGENT && m_pClientManager->GetSvrType(dwNum) == SERVER_AGENT)
				continue;
			if (netMsg->nServerType == NET_SERVER_FIELD && m_pClientManager->GetSvrType(dwNum) == SERVER_FIELD)
				continue;
			if (netMsg->nServerType == NET_SERVER_LOGIN && m_pClientManager->GetSvrType(dwNum) == SERVER_LOGIN)
				continue;
