	...
	DWORD dwClientID)
{
	...
	SQLCHAR szPetName[PETNAMESIZE + 1] = {0};
	SQLINTEGER cbPetName = SQL_NTS;
	...
	TIMESTAMP_STRUCT sPetSkinStartDate;
	SQLINTEGER cbPetSkinStartDate = SQL_NTS;
	...
	// Pet 의 인벤토리를 가져온다.
	if (GetPetInven(dwClientID, pPet, dwPetNum) == DB_ERROR)
		return DB_ERROR;

	// Pet 의 스킬을 가져온다.
	if (GetPetSkill(dwClientID, pPet, dwPetNum) == DB_ERROR)
		return DB_ERROR;
