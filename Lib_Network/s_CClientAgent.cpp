	...
	if (nRetCode == SOCKET_ERROR) 
	{
		...
		{	
			...
			if (nSendFieldError < 100)
			{
				...
				{
                 //Disable Log state for less console update.
...
void* CClientAgent::getRcvMsg(DWORD dwClient,bool &bBlocked)
{
	bool m_bBlocked(false);
	NET_MSG_GENERIC* pNmg = NULL;
	if (dwClient >= (DWORD) m_nMaxClient)	pNmg = (NET_MSG_GENERIC*)  m_pField[dwClient-m_nMaxClient].getRcvMsg(FALSE,m_bBlocked);
	// 클라이언트로 부터 온 메시지
	else									pNmg = (NET_MSG_GENERIC*)  m_pClient[dwClient].getRcvMsg(dwClient<NET_RESERVED_SLOT ? FALSE : TRUE,m_bBlocked);
	bBlocked = m_bBlocked;
	return pNmg;
}
...
void CClientAgent::SetClientValid (DWORD dwClient) // 계정 인증(login) 통과 설정.
{
	if (dwClient >= (DWORD) m_nMaxClient)	m_pClient[dwClient-m_nMaxClient].SetClientValid();
	else                                    m_pClient[dwClient].SetClientValid();
}

bool CClientAgent::GetValidClient (DWORD dwClient) // 계정 인증(login) 통과 설정.
{
	if (dwClient >= (DWORD) m_nMaxClient)	return m_pClient[dwClient-m_nMaxClient].GetValidClient();
	else                                    return m_pClient[dwClient].GetValidClient();
}
...
void CClientAgent::ResetClientCheck(DWORD dwClient)
{
	if (dwClient >= (DWORD)m_nMaxClient)	m_pField[dwClient - m_nMaxClient].ResetClientCheck();
	else									m_pClient[dwClient].ResetClientCheck();
}

void CClientAgent::SetClientCheck(DWORD dwClient)
{
	if (dwClient >= (DWORD)m_nMaxClient)	m_pField[dwClient - m_nMaxClient].SetClientCheck();
	else									m_pClient[dwClient].SetClientCheck();
}

BOOL CClientAgent::IsClientCheck(DWORD dwClient)
{
	if (dwClient >= (DWORD)m_nMaxClient)	return m_pField[dwClient - m_nMaxClient].IsClientCheck();
	else									return m_pClient[dwClient].IsClientCheck();
}

void CClientAgent::SetClientResult(DWORD dwClient, int nResult)
{
	if (dwClient >= (DWORD)m_nMaxClient)	m_pField[dwClient - m_nMaxClient].SetClientResult(nResult);
	else									m_pClient[dwClient].SetClientResult(nResult);
}

int CClientAgent::GetClientResult(DWORD dwClient)
{
	if (dwClient >= (DWORD)m_nMaxClient)	return m_pField[dwClient - m_nMaxClient].GetClientResult();
	else									return m_pClient[dwClient].GetClientResult();
}
