...
int CNetUser::CloseClient()
{
	...
	if (m_emOnline == NET_ONLINE)
	{
		...
		if (nRetCode == SOCKET_ERROR)
		{
			...
			{
				//Console Flood Needs to be Disabled. - Eduj kun
				//Socket Error for close
...
void CNetUser::CheckGarbageMsg(void *msg)
{
		...
		strlen( szTempChar ) != 0 )
	{
		...
		FILE* Detectlog;
		Detectlog = fopen("Blocklist.txt", "a+");
		fprintf(Detectlog, "%s\n", m_szIp);
		fclose(Detectlog);
	}
}
...
void* CNetUser::getRcvMsg( bool bClient, bool &bBlocked )
{	
	if (!IsOnline())
	{
