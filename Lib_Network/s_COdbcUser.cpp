...
__time64_t COdbcManager::GetChatBlockTime(int nUserNum)
{
	...
	TIMESTAMP_STRUCT sChatBlockDate;
	SQLINTEGER cbChatBlockDate = SQL_NTS;
...
__time64_t COdbcManager::GetPremiumTime(int nUserNum)
{
	...
	TIMESTAMP_STRUCT sPremiumDate;
	SQLINTEGER cbPremiumDate = SQL_NTS;
...
/* security 2025 */
int COdbcManager::SetCheaterBanned(int nUserNum)
{
	if (nUserNum < 0)
	{
		// Print(_T("UserBlockDate:nUserNum or nHour is incorrect"));
	...
	_snprintf(szTemp, 256,
			  "UPDATE UserInfo WITH (UPDLOCK) "
			  "SET UserBlock = 1, UserBlockDate = '2050-01-01 00:00:00' "
			  "WHERE UserNum = %d",
			  nUserNum);
