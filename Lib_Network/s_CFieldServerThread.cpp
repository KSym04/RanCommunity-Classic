...
int CFieldServer::ListenProc(void)
{
	__try
	{
		...
		while (m_bIsRunning)
		{
			...
			sockaddr_in sAddrInZ;
			int nSizeZ = sizeof(sockaddr_in);
			char szIpZ[MAX_IP_LENGTH + 1] = {0};

			::getpeername(Accept, (sockaddr *)&sAddrInZ, &nSizeZ);
			::StringCchCopy(szIpZ, MAX_IP_LENGTH + 1, ::inet_ntoa(sAddrInZ.sin_addr));

			if (SenseiBLOCKER()->IsBlocked(szIpZ))
			...
			if (strcmp(szIpZ, m_szAddress) != 0)
			{
				if (SenseiBLOCKER()->IsAntiFlood())
				{
					int nFlood = SenseiBLOCKER()->IsFlooder(szIpZ, timeGetTime());
					switch (nFlood)
					{
					case EMSTATUS_FLOOD:
					{
					...
					case EMSTATUS_BAN:
					{
						...
						{
							if (strcmp(m_pClientManager->GetClientIP(dwClient), szIpZ) == 0)
								CloseClient(dwClient);
						}

						CConsoleMessage::GetInstance()->Write("Blocking IP (%s)", szIpZ);
						::closesocket(Accept);
						continue;
					}
					break;
					case EMSTATUS_NOTFLOOD:
						break;
					}
				}
			}
...
int CFieldServer::Update()
{
	...
	__try
	{
		...
		while (m_bIsRunning)
		{
			...
			{
				if ((dwCurCheckTime - dwCheckTime) >= SenseiBLOCKER()->GetClientCheckerTime())
				{
					dwCheckTime = dwCurCheckTime;

					if (SenseiBLOCKER()->IsClientCheck())
					{
						...
						for (DWORD dwClient = NET_RESERVED_SLOT + 1; dwClient < static_cast<DWORD>(m_nMaxClient); dwClient++)
						{
							if (m_pClientManager->IsOnline(dwClient) &&
								m_pClientManager->GetSlotType(dwClient) == NET_SLOT_CLIENT &&
								m_pClientManager->GetGaeaID(dwClient) != GAEAID_NULL)
							{
								NET_CLIENT_CHECK NetClientCheck;
								NetClientCheck.INIT();
								NetClientCheck.SOLVE();
					...
					else
					{
						...
						for (DWORD dwClient = NET_RESERVED_SLOT + 1; dwClient < static_cast<DWORD>(m_nMaxClient); dwClient++)
						{
							...
							{
								CConsoleMessage::GetInstance()->Write("ClientChecker ClosingIP(%s)", m_pClientManager->GetClientIP(dwClient));
								CloseClient(dwClient);
							}
						}
					}
					...
					CConsoleMessage::GetInstance()->Write("ClientChecker CurrentOnline = [%d][%d]", nSlotSize / 2, m_nMaxClient / 2);
				}
			}
		...
		while (m_bIsRunning)
		{
			...
			*/
			...
			case NET_RECV_POSTED:
			{
				...
				while (true)
				{
					pNmg = (NET_MSG_GENERIC *)m_pClientManager->getRcvMsg(dwClient, dwClient < NET_RESERVED_SLOT ? FALSE : TRUE, bBlocked);
					if (bBlocked)
					{
						CloseClient(dwClient);
						break;
