...
#include "./MinLZ4.h" // LZ4 compression
...
const char *GARBAGE_DATA[] = {
	"K9IHANA", "L8IDUL", "M7HSET", "N6GNET", "O5FDASEOT" /*,
...
CRcvMsgBuffer::CRcvMsgBuffer()
	: m_pRcvBuffer(NULL), m_pOneMsg(NULL), m_pDecompressBuffer(NULL), m_nRcvSize(0)
{
	// Receive buffer can hold multiple messages already combined
	m_pRcvBuffer = new BYTE[NET_DATA_CLIENT_MSG_BUFSIZE];
	// Single message working buffer (legacy size)
	m_pOneMsg = new BYTE[NET_DATA_BUFSIZE];
	// Decompression buffer: must be able to hold the largest possible expanded block.
	// Use NET_DATA_CLIENT_MSG_BUFSIZE to avoid overflow when a combined compressed block expands.
	m_pDecompressBuffer = new BYTE[NET_DATA_CLIENT_MSG_BUFSIZE];
...
void *CRcvMsgBuffer::getMsg(bool bClient)
{
	...
	if (NET_MSG_COMPRESS == pNmg->nType)
	{
		...
		{
			// If this packet originated from an LZO-era client/server pair but server upgraded header,
			// we might receive bCompress=true yet data not LZ4-compressed (still plain combined messages).
			// Heuristic: legacy path had no dwOrigSize field (was usCount). If dwOrigSize==m_dwPos or 0 and
			// compressed payload begins with a plausible NET_MSG_GENERIC of next message, treat as uncompressed.
			if (pNmc->dwOrigSize == 0)
			{
				NET_MSG_GENERIC *pInner = (NET_MSG_GENERIC *)((BYTE *)pNmc + sizeof(NET_COMPRESS));
				if (pNmc->nmg.dwSize > sizeof(NET_COMPRESS) + sizeof(NET_MSG_GENERIC) && pInner->dwSize <= NET_DATA_BUFSIZE && pInner->dwSize >= sizeof(NET_MSG_GENERIC))
				{
#ifdef _DEBUG
					OutputDebugStringA("[LZ4] Treating legacy 'compressed' packet as plain (compat mode)\n");
#endif
					int nMoveSizeLegacy = m_nRcvSize - (int)sizeof(NET_COMPRESS);
					if (nMoveSizeLegacy < 0)
					{
						::LeaveCriticalSection(&m_CriticalSection);
						return NULL;
					}
					::MoveMemory(m_pRcvBuffer, m_pRcvBuffer + sizeof(NET_COMPRESS), nMoveSizeLegacy);
					m_nRcvSize = nMoveSizeLegacy;
					void *pOneMsgCompat = getOneMsg(bClient);
					::LeaveCriticalSection(&m_CriticalSection);
					return pOneMsgCompat;
				}
			}
			// 압축된 메시지이다...
			// 전체 메시지 크기는...
			int nCompressSize = (int)pNmc->nmg.dwSize;
			// Attempt to interpret dwOrigSize safely even with legacy clients (which didn't set it)
			DWORD dwDeclaredOrig = pNmc->dwOrigSize;
			int nDeCompressSize = NET_DATA_CLIENT_MSG_BUFSIZE;
			if (dwDeclaredOrig > 0 && dwDeclaredOrig <= NET_DATA_CLIENT_MSG_BUFSIZE)
			{
				nDeCompressSize = (int)dwDeclaredOrig;
			}
			//		// 압축을 해제한다.
			//		/*
			...
			// LZ4 decompression (parameter order: input, inputSize, outputBuffer, outputSize[IN/OUT])
			int nResult = CMinLZ4::GetInstance().lz4DeCompress(
				(byte *)pNmc + sizeof(NET_COMPRESS),
				pNmc->nmg.dwSize - sizeof(NET_COMPRESS),
				m_pDecompressBuffer,
				nDeCompressSize);

			if (nResult == CMinLZ4::MINLZ4_SUCCESS)
			{
				// Validate decompressed size against buffer constraints and basic ratio (defensive)
				if (nDeCompressSize <= 0 || nDeCompressSize > NET_DATA_CLIENT_MSG_BUFSIZE)
				{
#ifdef _DEBUG
					OutputDebugStringA("[LZ4] Decompressed size invalid/out-of-range\n");
#endif
					// Treat as failure path below
				...
				// Optional expansion ratio check (prevent zip-bomb style) allow up to 8x expansion
				if ((nDeCompressSize > (nCompressSize * 8)) && nCompressSize > 0)
				{
#ifdef _DEBUG
					OutputDebugStringA("[LZ4] Expansion ratio exceeds 8x limit, dropping packet\n");
#endif
					int nMoveSize = m_nRcvSize - nCompressSize;
					if (nMoveSize < 0)
					{
				...
				// Successful decompression integration back into receive stream
				int nMoveSize = m_nRcvSize - nCompressSize;
				if (nMoveSize < 0)
				{
					::LeaveCriticalSection(&m_CriticalSection);
...
#ifdef _DEBUG
				if (pNmc->dwOrigSize && pNmc->dwOrigSize != (DWORD)nDeCompressSize)
				{
					OutputDebugStringA("[LZ4] Warning: dwOrigSize header mismatch after decompress\n");
				}
#endif
			}
			else
			{
				...
				int nMoveSize = m_nRcvSize - nCompressSize;
#ifdef _DEBUG
				OutputDebugStringA("[LZ4] Decompression failed: ");
				OutputDebugStringA(CMinLZ4::GetInstance().getErrorString().c_str());
				OutputDebugStringA("\n");
#endif
