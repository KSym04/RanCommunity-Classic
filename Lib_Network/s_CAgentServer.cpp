...
CAgentServer::CAgentServer(HWND hWnd, HWND hEditBox, HWND hEditBoxInfo, HWND hEditBoxStats)
	: CServer(hWnd, hEditBox, hEditBoxInfo, hEditBoxStats), m_pClientManager(NULL), m_hCheckStrDLL(NULL), m_pCheckString(NULL)
{
	m_bHeartBeat = false;
...
int CAgentServer::StartDbManager()
{
	...
	if (StartDatabaseThread() == NET_ERROR)
		return NET_ERROR;
	if (StartLogDatabaseThread() == NET_ERROR)
		return NET_ERROR;
	if (StartUserDatabaseThread() == NET_ERROR)
		return NET_ERROR;
	if (StartWebDatabaseThread() == NET_ERROR)
		return NET_ERROR;
...
int CAgentServer::Start()
{
	...
	if (StartIOCPList() == NET_ERROR)
		return NET_ERROR;
	if (StartMsgManager() == NET_ERROR)
		return NET_ERROR;
...
int CAgentServer::ExecuteCommand(char *strCmd)
{
	...
	while (token != NULL)
	{
		...
		if (strcmp(token, "c") == 0)
		{
			CConsoleMessage::GetInstance()->Write("%d %d", (int)m_pClientManager->GetCurrentClientNumber(),
...
int CAgentServer::CheckTracingUser(const char *pUserAccount, const int nUserNum)
{
	...
	int i;
	for (i = 0; static_cast<size_t>(i) < m_vecTracingData.size(); i++)
	{
	...
	if ((dwCurrentTime - m_dwHeartBeatTime) > SESSION_CHECK_TIME)
	{
		...
		if (m_bGameGuardInit == true && m_bGameGuardAuth == true)
		{
			...
			if (m_bHeartBeat)
			{
				...
				for (dwN = NET_RESERVED_SLOT; dwN < static_cast<DWORD>(m_nMaxClient); ++dwN)
				{
