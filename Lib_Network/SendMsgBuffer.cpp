...
#include "./MinLZ4.h" // switched from MinLzo to MinLZ4
...
CSendMsgBuffer::CSendMsgBuffer(/* CMinLzo* pMinLzo */)
	: m_dwPos(0), m_pBuffer(NULL), m_pSendBuffer(NULL), m_usCount(0)
// m_pMinLzo(pMinLzo)
{
...
int CSendMsgBuffer::getSendSize()
{
	...
	*/
	...
	// TEMPORARY: Disable compression while client still expects LZO.
	// Once client is updated to LZ4, restore heuristic or feature toggle.
	bool bForceNoCompress = true; //(m_dwPos < 64) || (m_usCount <= 1);

	nOutLength = m_dwPos; // provide max for output, will be overwritten with compressed size
	if (!bForceNoCompress)
	{
		nCompressResult = CMinLZ4::GetInstance().lz4Compress(
			m_pBuffer,
			m_dwPos,
			m_pSendBuffer + sizeof(NET_COMPRESS),
			nOutLength);
	...
	{
		nCompressResult = CMinLZ4::MINLZ4_CAN_NOT_COMPRESS; // treat as no compression path
	}

	if (nCompressResult == CMinLZ4::MINLZ4_SUCCESS)
	{
		// 압축성공, 데이터 합쳐서 재구성
		...
		pNmc->bCompress = true;
		pNmc->usCount = m_usCount; // legacy field population
		pNmc->dwOrigSize = m_dwPos;
	}
	else
	{
		...
		pNmc->usCount = m_usCount;	// legacy field population
		pNmc->dwOrigSize = m_dwPos; // uncompressed original
	}
