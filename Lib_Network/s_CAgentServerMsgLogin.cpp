...
{
	if (m_nServiceProvider != SP_THAILAND || pMsg == NULL)
		return;
...
void CAgentServer::ChinaMsgLogin(MSG_LIST *pMsg)
{
	...
	if (m_nServiceProvider != SP_CHINA || pMsg == NULL)
		return;
...
void CAgentServer::MsgLogIn(MSG_LIST *pMsg)
{
	...
	*/
	...
	if (_tcscmp(szEncrypt, m_szEncrypt))
	{
		...
		// SECURITY / STABILITY FIX (2025-08-21, Eifelzocker fix):
		// Previously the server left the socket open after an encryption key mismatch.
		// A flood of invalid login packets could accumulate many half-auth sessions,
		// increasing processing time and triggering heartbeat timeouts for valid users,
		// leading to mass disconnects. We now actively close the offending client.
...
void CAgentServer::GsMsgLogin(MSG_LIST *pMsg)
{
	...
	if (_tcscmp(szEncrypt, m_szEncrypt))
	{
		...
		// Eifelzocker fix: Close invalid session to prevent accumulation of bad connections.
		CloseClient(dwClient);
...
{
	if (m_nServiceProvider != SP_JAPAN)
		return;
...
void CAgentServer::JapanMsgLogin(MSG_LIST *pMsg)
{
	...
	if (_tcscmp(szEncrypt, m_szEncrypt))
	{
		...
		// Eifelzocker fix: Close invalid session to avoid resource leak / heartbeat starvation.
		CloseClient(dwClient);
...
void CAgentServer::JapanWRFailed(DWORD dwClient, int nResult)
{
	...
	if (HTTP_STATUS_BAD_GATEWAY == nResult)
		nlfd.nResult = EM_LOGIN_FB_SUB_INCORRECT; // ID, PW 불일치로 로그인 실패
	else if (HTTP_STATUS_ALREADY_LOGIN == nResult)
		nlfd.nResult = EM_LOGIN_FB_SUB_DUP; // 중복로그인
	else
		nlfd.nResult = EM_LOGIN_FB_SUB_SYSTEM; // 시스템 에러로 로그인 실패
