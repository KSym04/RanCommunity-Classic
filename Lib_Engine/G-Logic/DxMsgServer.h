...
public:
	...
	{
		GASSERT(0 && "CloseClient()가 제정의되지 않은 상태에서 호출됨");
		return 0;
	}
...
public:
	...
	{
		GASSERT(0 && "IsFieldOnline()가 제정의되지 않은 상태에서 호출됨");
		return TRUE;
	}
	virtual bool IsSlotOnline(DWORD dwClient)
	{
		GASSERT(0 && "IsSlotOnline()가 제정의되지 않은 상태에서 호출됨");
		return TRUE;
	}

	// Security: returns the remote IP address string for a connected client slot.
	// Used to log attacker IPs when exploit attempts are detected.
	virtual char *GetClientIP(DWORD dwClient) { return ""; }
};
