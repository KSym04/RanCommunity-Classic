...
CNPCItemExchangeWindow::CNPCItemExchangeWindow()
	: m_pTextHead1(NULL), m_pTextHead2(NULL), m_pButtonPrev(NULL), m_pButtonNext(NULL), m_pTextPage(NULL), m_sidNPC(NATIVEID_NULL()), m_nCurrentPage(1), m_nMaxPage(1)
{
...
void CNPCItemExchangeWindow::ShowPage(int nPage)
{
	...
	{
		if (static_cast<size_t>(i) >= m_vecList.size())
			continue;
