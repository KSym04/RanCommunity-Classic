...
namespace NS_ITEMINFO
{
	...
	void AddItemTurnInfo(const SITEMCUSTOM &sItemCustom, const BOOL bInMarket, const BOOL bInPrivateMarket)
	{
		...
		if (pItemData->sDrugOp.wPileNum > 1)
		{
			...
			if (bInMarket)
				wTurnNum = pItemData->GETAPPLYNUM();
	...
	void AddInfoSkillItem(const SITEMCUSTOM &sItemCustom)
	{
		...
		if (pSkill)
		{
			//	기본 정보
			{
				...
				if (pSkill->m_sLEARN.dwCLASS & GLCC_BRAWLER_M)
					strText.AppendFormat("%s", COMMENT::CHARCLASS[0].c_str());
				if (pSkill->m_sLEARN.dwCLASS & GLCC_BRAWLER_W)
					strText.AppendFormat("%s", COMMENT::CHARCLASS[6].c_str());
				if (pSkill->m_sLEARN.dwCLASS & GLCC_SWORDSMAN_M)
					strText.AppendFormat("%s", COMMENT::CHARCLASS[1].c_str());
				if (pSkill->m_sLEARN.dwCLASS & GLCC_SWORDSMAN_W)
					strText.AppendFormat("%s", COMMENT::CHARCLASS[7].c_str());
				if (pSkill->m_sLEARN.dwCLASS & GLCC_ARCHER_M)
					strText.AppendFormat("%s", COMMENT::CHARCLASS[8].c_str());
				if (pSkill->m_sLEARN.dwCLASS & GLCC_ARCHER_W)
					strText.AppendFormat("%s", COMMENT::CHARCLASS[2].c_str());
				if (pSkill->m_sLEARN.dwCLASS & GLCC_SHAMAN_M)
					strText.AppendFormat("%s", COMMENT::CHARCLASS[9].c_str());
				if (pSkill->m_sLEARN.dwCLASS & GLCC_SHAMAN_W)
					strText.AppendFormat("%s", COMMENT::CHARCLASS[3].c_str());
				if (pSkill->m_sLEARN.dwCLASS & GLCC_EXTREME_M)
					strText.AppendFormat("%s", COMMENT::CHARCLASS[4].c_str());
				if (pSkill->m_sLEARN.dwCLASS & GLCC_EXTREME_W)
					strText.AppendFormat("%s", COMMENT::CHARCLASS[5].c_str());
				if (pSkill->m_sLEARN.dwCLASS & GLCC_GUNNER_M)
					strText.AppendFormat("%s", COMMENT::CHARCLASS[10].c_str());
				if (pSkill->m_sLEARN.dwCLASS & GLCC_GUNNER_W)
					strText.AppendFormat("%s", COMMENT::CHARCLASS[11].c_str());
				if (pSkill->m_sLEARN.dwCLASS & GLCC_ASSASSIN_M)
					strText.AppendFormat("%s", COMMENT::CHARCLASS[12].c_str());
				if (pSkill->m_sLEARN.dwCLASS & GLCC_ASSASSIN_W)
					strText.AppendFormat("%s", COMMENT::CHARCLASS[13].c_str());
				if (pSkill->m_sLEARN.dwCLASS & GLCC_TRICKER_M)
					strText.AppendFormat("%s", COMMENT::CHARCLASS[14].c_str());
				if (pSkill->m_sLEARN.dwCLASS & GLCC_TRICKER_W)
					strText.AppendFormat("%s", COMMENT::CHARCLASS[15].c_str());
	...
	void AddTextAddValue(const SITEMCUSTOM &sItemCustom, const ITEM::SSUIT &sSUIT)
	{
		...
		for (int i = 0; i < ITEM::SSUIT::ADDON_SIZE; ++i)
		{
			...
			if (emTYPE != EMADD_NONE)
			{
				...
				case EMADD_HITRATE:
					arrVALUE[EMADD_HITRATE] += nVALUE;
					break;
				case EMADD_AVOIDRATE:
					arrVALUE[EMADD_AVOIDRATE] += nVALUE;
					break;
				case EMADD_DAMAGE:
					arrVALUE[EMADD_DAMAGE] += nVALUE;
					break;
				case EMADD_DEFENSE:
					arrVALUE[EMADD_DEFENSE] += nVALUE;
					break;
				case EMADD_STATS_POW:
					arrVALUE[EMADD_STATS_POW] += nVALUE;
					break;
				case EMADD_STATS_STR:
					arrVALUE[EMADD_STATS_STR] += nVALUE;
					break;
				case EMADD_STATS_SPI:
					arrVALUE[EMADD_STATS_SPI] += nVALUE;
					break;
				case EMADD_STATS_DEX:
					arrVALUE[EMADD_STATS_DEX] += nVALUE;
					break;
				case EMADD_STATS_INT:
					arrVALUE[EMADD_STATS_INT] += nVALUE;
					break;
				case EMADD_STATS_STA:
					arrVALUE[EMADD_STATS_STA] += nVALUE;
					break;
				case EMADD_MAX_CP:
					arrVALUE[EMADD_MAX_CP] += nVALUE;
					break; /*item max CP, Juver, 2018/07/09 */
				};
			}
		}
			...
			for (int i = 0; i < ITEM::SSUIT::ADDON_SIZE; ++i)
			{
				...
				if (emTYPE != EMADD_NONE)
				{
					...
					case EMADD_STATS_STA:
						...
						break;
					case EMADD_PA:
						arrVALUE[EMADD_PA] += nVALUE;
						break;
					case EMADD_SA:
						arrVALUE[EMADD_SA] += nVALUE;
						break;
					case EMADD_MA:
	...
	{
		if (m_sItemCustomBACK == sItemCustom)
			return;
			  ...
			  BOOL bCurrencyShop /*= FALSE*/, DWORD dwCurrencyShopNpcID /*= 0*/, DWORD dwCurrencyShopItemID /*= 0*/, DWORD dwMarketChannel /*= 0*/)
	{
		...
		/*item link, Juver, 2017/07/31 */
		/*product item, Juver, 2017/10/15 */
		if (m_sItemCustomBACK == sItemCustom_base && m_bShopOpenBACK == bShopOpen && m_bInMarketBACK == bInMarket && m_bInPrivateMarketBACK == bInPrivateMarket && m_bIsWEAR_ITEMBACK == bIsWEAR_ITEM && m_wPosXBACK == wPosX && m_wPosYBACK == wPosY && m_bNPCShopBACK == bNPCShop && m_wNPCShopTypeBACK == wNPCShopType && m_bItemLinkBACK == bItemLink && m_bInInventoryBACK == bInInventory && m_bInCompoundResultBACK == bInCompoundResult && m_bCompoundGenerateRandomOptionBACK == bCompoundGenerateRandomOption && m_bCurrencyShopBACK == bCurrencyShop && m_dwCurrencyShopItemIDBACK == dwCurrencyShopItemID && m_dwCurrencyShopNpcIDBACK == dwCurrencyShopNpcID && m_dwMarketChannelBACK == dwMarketChannel)
			return;
		...
		if (dwUSERLVL > NSUSER_TYPE::USER_TYPE_GM3)
		{
			...
			if (pItemData->sGenerateOp.dwSpecID)
				AddTextNoSplit(strGen, pItemData->sGenerateOp.bGenEnable ? NS_UITEXTCOLOR::ENABLE : NS_UITEXTCOLOR::DISABLE);
		}
...

		{
						...
						if (pCrowData != NULL)
						{
							...
							if (strCrowSale.size())
							{
								...
								if (pCrowSale)
								{
									...
									{
										dwNpcSellPrice = static_cast<DWORD>(dwNpcPrice);
			...
			case ITEM_PET_CARD:
			{
				...
				if (iter == GLGaeaClient::GetInstance().GetCharacter()->m_mapPETCardInfo.end())
				{
					...
					if (iter == GLGaeaClient::GetInstance().GetCharacter()->m_mapPETCardInfoTemp.end())
						break;
				}
			...
			case ITEM_RANDOM_OPTION_CARD:
			{
				...
				if ((EMR_OPT_NULL < nRandOptType) && (nRandOptType < EMR_OPT_SIZE))
				{
					...
					if (fVal != 0.0f)
					{
						...
						if (sitem_custom_copy.IsPerRandOpt(nRandOptType))
							strOptionText += _T('%');
			...
			{ // itemflags
				...
				if (pitem_costume)
				{
					if (!pitem_costume->sBasicOp.IsSALE())
						bSale = false;
					if (!pitem_costume->sBasicOp.IsEXCHANGE())
						bExchange = false;
					if (!pitem_costume->sBasicOp.IsLocker())
						bLocker = false;
					if (!pitem_costume->sBasicOp.IsClubLocker())
						bClubLocker = false;
					if (!pitem_costume->sBasicOp.IsTHROW())
						bThrow = false;
				}
			...
			if (!bWrapped)
			{
				...
				if (sItemCustom.nidDISGUISE != NATIVEID_NULL())
					bWrappable = FALSE;
				if (!sItemCustom.IsGM_GENITEM() && pItemData->sBasicOp.IsEXCHANGE())
					bWrappable = FALSE;
