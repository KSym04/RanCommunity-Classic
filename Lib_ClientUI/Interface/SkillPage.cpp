...
CSkillPage::CSkillPage() : m_pScrollBar(NULL),
						   m_nCurPos(-1)
{
...
void CSkillPage::SetSkillSlotRender(int nStartIndex, int nTotal)
{
	...
	for (int i = nStartIndex; static_cast<size_t>(i) < m_vecViewSkill.size(); i++)
	{
		if (i >= m_nMaxSkillCnt)
			break;
		if (i >= nTotal)
	...
	if (m_vecViewSkill.size() < static_cast<size_t>(nTotal))
	{
		int iMaxLackTotal = nTotal - static_cast<int>(m_vecViewSkill.size());
	...
	for (size_t i = nTotal; i < m_vecViewSkill.size(); i++)
	{
		...
		if (m_pSkillSlotArray[iSlotNum])
			m_pSkillSlotArray[iSlotNum]->SetVisibleSingle(FALSE);
	}
}
