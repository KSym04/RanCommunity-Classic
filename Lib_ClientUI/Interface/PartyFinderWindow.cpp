...
CPartyFinderWindow::CPartyFinderWindow()
	: m_nSelectIndexType(-1), m_nCurrentPage(1), m_nMaxPage(1), m_dwSelectedIndex(SPARTY_FINDER_SEARCH_INDEX_NULL), m_pListTextBox(NULL), m_pListScrollBar(NULL), m_pButtonSearch(NULL), m_pButtonPrev(NULL), m_pButtonNext(NULL), m_pTextPage(NULL)
{
...
void CPartyFinderWindow::ShowSlot(int nPage)
{
	...
	{
		if (static_cast<size_t>(i) >= m_vecResult.size())
			continue;
