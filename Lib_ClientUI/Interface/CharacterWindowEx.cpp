...
void CCharacterWindow::SetEXP(const GLCHARLOGIC_CLIENT &sCharData)
{
	...
	if (m_pEXPText)
		nIndex = m_pEXPText->AddTextNoSplit(strCombine, NS_UITEXTCOLOR::WHITE);

	strCombine.Format(" / ");
	if (m_pEXPText)
		m_pEXPText->AddString(nIndex, strCombine, NS_UITEXTCOLOR::LIGHTGREY);

	strCombine.Format("%I64d", lMAX);
	if (m_pEXPText)
		m_pEXPText->AddString(nIndex, strCombine, NS_UITEXTCOLOR::WHITE);
...
void CCharacterWindow::SetPow(const GLCHARLOGIC_CLIENT &sCharData)
{
	...
	if (sITEM.wPow)
		dwColor = NS_UITEXTCOLOR::PLUSOPTION;
...
void CCharacterWindow::SetPointControl(BOOL bPointUsable)
{
	//	Point
	if (m_pPointDisable)
		m_pPointDisable->SetVisibleSingle(!bPointUsable);
	if (m_pPointEnable)
		m_pPointEnable->SetVisibleSingle(bPointUsable);

	//	Stats Button
	if (m_pPowButton)
		m_pPowButton->SetVisibleSingle(bPointUsable);
	if (m_pDexButton)
		m_pDexButton->SetVisibleSingle(bPointUsable);
	if (m_pSpiButton)
		m_pSpiButton->SetVisibleSingle(bPointUsable);

	if (m_pStrButton)
		m_pStrButton->SetVisibleSingle(bPointUsable);
	if (m_pStaButton)
		m_pStaButton->SetVisibleSingle(bPointUsable);

	if (m_pPowDiaableButton)
		m_pPowDiaableButton->SetVisibleSingle(!bPointUsable);
	if (m_pDexDiaableButton)
		m_pDexDiaableButton->SetVisibleSingle(!bPointUsable);
	if (m_pSpiDiaableButton)
		m_pSpiDiaableButton->SetVisibleSingle(!bPointUsable);

	if (m_pStrDiaableButton)
		m_pStrDiaableButton->SetVisibleSingle(!bPointUsable);
	if (m_pStaDiaableButton)
		m_pStaDiaableButton->SetVisibleSingle(!bPointUsable);
}
...
void CCharacterWindow::SetPoint(const GLCHARLOGIC_CLIENT &sCharData)
{
	...
	if (m_pPointText)
		m_pPointText->SetOneLineText(strCombine, NS_UITEXTCOLOR::GOLD);
}
...
void CCharacterWindow::SetDamage(const GLCHARLOGIC_CLIENT &sCharData)
{
	...
	if (m_pDamageText)
		m_pDamageText->SetOneLineText(strCombine, NS_UITEXTCOLOR::WHITE);
}
...
void CCharacterWindow::SetDefense(const GLCHARLOGIC_CLIENT &sCharData)
{
	...
	if (m_pDefenseText)
		m_pDefenseText->SetOneLineText(strCombine, NS_UITEXTCOLOR::WHITE);
}
...
void CCharacterWindow::SetReqPa(const GLCHARLOGIC_CLIENT &sCharData)
{
	...
	if (m_pReqPaText)
		m_pReqPaText->SetOneLineText(strCombine, NS_UITEXTCOLOR::WHITE);
}
...
void CCharacterWindow::SetReqSa(const GLCHARLOGIC_CLIENT &sCharData)
{
	...
	if (m_pReqSaText)
		m_pReqSaText->SetOneLineText(strCombine, NS_UITEXTCOLOR::WHITE);
}
...
void CCharacterWindow::SetReqMa(const GLCHARLOGIC_CLIENT &sCharData)
{
	...
	if (m_pReqMaText)
		m_pReqMaText->SetOneLineText(strCombine, NS_UITEXTCOLOR::WHITE);
}
...
void CCharacterWindow::SetHitRate(const GLCHARLOGIC_CLIENT &sCharData)
{
	...
	if (m_pHitRateText)
		m_pHitRateText->SetOneLineText(strCombine, NS_UITEXTCOLOR::WHITE);
}
...
void CCharacterWindow::SetHP(const GLCHARLOGIC_CLIENT &sCharData)
{
	...
	if (m_pHPText)
		m_pHPText->SetOneLineText(strCombine, NS_UITEXTCOLOR::WHITE);
}
...
void CCharacterWindow::SetMP(const GLCHARLOGIC_CLIENT &sCharData)
{
	...
	if (m_pMPText)
		m_pMPText->SetOneLineText(strCombine, NS_UITEXTCOLOR::WHITE);
}
...
void CCharacterWindow::SetSP(const GLCHARLOGIC_CLIENT &sCharData)
{
	...
	if (m_pSPText)
		m_pSPText->SetOneLineText(strCombine, NS_UITEXTCOLOR::WHITE);
}
...
void CCharacterWindow::SetAvoidRate(const GLCHARLOGIC_CLIENT &sCharData)
{
	...
	if (m_pAvoidRateText)
		m_pAvoidRateText->SetOneLineText(strCombine, NS_UITEXTCOLOR::WHITE);
}
...
void CCharacterWindow::SetFireResi(const GLCHARLOGIC_CLIENT &sCharData)
{
	...
	if (m_pFireText)
		m_pFireText->SetOneLineText(strCombine, NS_UITEXTCOLOR::WHITE);
}
...
void CCharacterWindow::SetColdResi(const GLCHARLOGIC_CLIENT &sCharData)
{
	...
	if (m_pColdText)
		m_pColdText->SetOneLineText(strCombine, NS_UITEXTCOLOR::WHITE);
}
...
void CCharacterWindow::SetElectricResi(const GLCHARLOGIC_CLIENT &sCharData)
{
	...
	if (m_pElectricText)
		m_pElectricText->SetOneLineText(strCombine, NS_UITEXTCOLOR::WHITE);
}
...
void CCharacterWindow::SetPoisonResi(const GLCHARLOGIC_CLIENT &sCharData)
{
	...
	if (m_pPoisonText)
		m_pPoisonText->SetOneLineText(strCombine, NS_UITEXTCOLOR::WHITE);
}
...
void CCharacterWindow::SetSpiritResi(const GLCHARLOGIC_CLIENT &sCharData)
{
	...
	if (m_pSpiritText)
		m_pSpiritText->SetOneLineText(strCombine, NS_UITEXTCOLOR::WHITE);
}
...
void CCharacterWindow::SetCP(const GLCHARLOGIC_CLIENT &sCharData)
{
	...
	if (m_pCPText)
		m_pCPText->SetOneLineText(strCombine, NS_UITEXTCOLOR::WHITE);
}
...
void CCharacterWindow::LoadBadge(const GLCHARLOGIC_CLIENT &sCharData)
{
	...
	for (; pos != end; ++pos)
	{
		...
		if (!pactivity)
			continue;
		if (!pactivity->bRewardBadge)
			continue;
		if (!pactivity->strBadgeString.size())
			continue;
...
void CCharacterWindow::SetReborn(const GLCHARLOGIC_CLIENT &sCharData)
{
	...
	const WORD wReborn = static_cast<WORD>(sCharData.m_dwReborn);
