...
CLargeMapWindow::CLargeMapWindow()
	: m_pMousePosText(NULL), m_pMousePosBack(NULL), m_pScrollBarH(NULL), m_pScrollBarV(NULL), m_pMap(NULL), m_pPlayerMark(NULL), m_pLBThumb(NULL), m_pRBThumb(NULL), m_pTargetMark(NULL), m_iTargetX(INT_MIN), m_iTargetY(INT_MIN)

	  /* map window mark, Juver, 2020/03/31 */
	  ,
	  m_NEWID(MAP_MARK), m_pMarkDummy(NULL)
{
	...
	if (bFirstControl)
	{
		...
		{ //	세로 스크롤
			...
			if (fPercentV != m_fPercentV_BACK)
			{
				...
				if (m_nVIEW_START_Y < 0)
					m_nVIEW_START_Y = 0;
				if (m_nVIEW_START_Y + m_nVIEW_SIZE_Y > m_nMAP_TEXSIZE_Y)
					m_nVIEW_START_Y = m_nMAP_TEXSIZE_Y - m_nVIEW_SIZE_Y;
			}
		}
		...
		{ //	가로 스크롤
			...
			if (fPercentH != m_fPercentH_BACK)
			{
				...
				if (m_nVIEW_START_X < 0)
					m_nVIEW_START_X = 0;
				if (m_nVIEW_START_X + m_nVIEW_SIZE_X > m_nMAP_TEXSIZE_X)
					m_nVIEW_START_X = m_nMAP_TEXSIZE_X - m_nVIEW_SIZE_X;
			}
		}
...
void CLargeMapWindow::TranslateUIMessage(UIGUID cID, DWORD dwMsg)
{
	...
	break;
	case LARGEMAP_IMAGE:
		TranslateMeg(cID, dwMsg, m_pMap);
		break;
	case LARGEMAP_LB_THUMB:
		TranslateMegThumb(cID, dwMsg, m_pLBThumb);
		break;
	case LARGEMAP_RB_THUMB:
		TranslateMegThumb(cID, dwMsg, m_pRBThumb);
		break;
	}
}
...
void CLargeMapWindow::SetVisibleSingle(BOOL bVisible)
{
	...
	UPDATE_GUILD_INFO(bVisible != FALSE);
...
void CLargeMapWindow::DoCHARPOS_OnCENTER_InVIEW(int nMapPosX, int nMapPosY)
{
	...
	if (m_nVIEW_START_Y < 0)
		m_nVIEW_START_Y = 0;
	if (m_nMAP_TEXSIZE_X - m_nVIEW_SIZE_X < m_nVIEW_START_X)
		m_nVIEW_START_X = m_nMAP_TEXSIZE_X - m_nVIEW_SIZE_X;
	if (m_nMAP_TEXSIZE_Y - m_nVIEW_SIZE_Y < m_nVIEW_START_Y)
		m_nVIEW_START_Y = m_nMAP_TEXSIZE_Y - m_nVIEW_SIZE_Y;

	{
		...
		if (m_vScrollGap.x == 0.0f)
			pThumbFrame->SetPercent(0.0f);
		else
			pThumbFrame->SetPercent(m_nVIEW_START_X / m_vScrollGap.x);
	}

	{
		...
		if (m_vScrollGap.y == 0.0f)
			pThumbFrame->SetPercent(0.0f);
		else
			pThumbFrame->SetPercent(m_nVIEW_START_Y / m_vScrollGap.y);
	}
}
