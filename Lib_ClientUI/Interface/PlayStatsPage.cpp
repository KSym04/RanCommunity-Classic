...
CPlayStatsPage::CPlayStatsPage()
	: m_fUpdateTimer(0.0f), m_bFirstLBUP(FALSE)

	  ,
	  m_pTextBasicInformationName(NULL), m_pTextBasicInformationSchool(NULL), m_pTextBasicInformationClass(NULL), m_pTextBasicInformationGuild(NULL), m_pImageBasicInfoClub(NULL)
	  ...
	  ,
	  m_pTextPlayInfoPlayTime(NULL), m_pTextPlayInfoPlayPoints(NULL)

	  ,
	  m_pTextCombatRecordTotalKills(NULL), m_pTextCombatRecordTotalDeaths(NULL), m_pTextCombatRecordTotalResu(NULL)

	  ,
	  m_pTextCombatRecordTyrannyKills(NULL), m_pTextCombatRecordTyrannyDeaths(NULL), m_pTextCombatRecordTyrannyResu(NULL)

	  ,
	  m_pTextCombatRecordClubWarsKills(NULL), m_pTextCombatRecordClubWarsDeaths(NULL), m_pTextCombatRecordClubWarsResu(NULL)

	  ,
	  m_pTextCombatRecordCaptureTheFlagKills(NULL), m_pTextCombatRecordCaptureTheFlagDeaths(NULL), m_pTextCombatRecordCaptureTheFlagResu(NULL)

	  ,
	  m_pTextCombatRecordClubDeathMatchKills(NULL), m_pTextCombatRecordClubDeathMatchDeaths(NULL), m_pTextCombatRecordClubDeathMatchResu(NULL)

	  ,
	  m_pTextCombatRecordSchoolWarsKills(NULL), m_pTextCombatRecordSchoolWarsDeaths(NULL), m_pTextCombatRecordSchoolWarsResu(NULL)

	  ,
	  m_pTextCombatRecordPKMapKills(NULL), m_pTextCombatRecordPKMapDeaths(NULL), m_pTextCombatRecordPKMapResu(NULL)

	  ,
	  m_pTextCombatRecordPVEKills(NULL), m_pTextCombatRecordPVEDeaths(NULL), m_pTextCombatRecordPVEGoldLoot(NULL)

	  // MMR - JX
	  ,
	  m_pTextMMRRankName(NULL), m_pTextMMRPoints(NULL)
...
HRESULT CPlayStatsPage::Render(LPDIRECT3DDEVICEQ pd3dDevice)
{
	...
	if (m_pImageBasicInfoClub)
		m_pImageBasicInfoClub->SetVisibleSingle(FALSE);
	{
	...
	if (m_pImageBasicInfoClub)
		m_pImageBasicInfoClub->SetVisibleSingle(TRUE);
...
{
	if (!m_pMMRProgressBar || !m_pMMRProgressText)
		return;
