...
CPandoraBoxWindow::CPandoraBoxWindow(GLGaeaClient *pGaeaClient, CInnerInterface *pInterface)
	: m_pGaeaClient(pGaeaClient), m_pInterface(pInterface), m_pListTextBox(NULL), m_pListScrollBar(NULL), m_pItemListScrollBar(NULL), m_pButtonPurchase(NULL), m_pTextCharEPoint(NULL), m_pTextCharGoldPoint(NULL), m_pButtonClear(NULL), m_pTextEPoint(NULL), m_pTextGoldPoint(NULL), m_sSelectItem(NATIVEID_NULL()), m_nCurPos(-1)
{
...
void CPandoraBoxWindow::Update(int x, int y, BYTE LB, BYTE MB, BYTE RB, int nScroll, float fElapsedTime, BOOL bFirstControl)
{
	...
	if (m_pItemListScrollBar)
	{
		...
		if (nTotal < PANDORA_BOX_MAX_ITEM)
			return;
...
void CPandoraBoxWindow::ShowItem()
{
	...
	{
		if (static_cast<size_t>(i) >= GLCONST_CHAR::vecPandoraBoxes.size())
			continue;
		if (m_pItemSlot[i])
		{
...
void CPandoraBoxWindow::ResetResultsSlotRender()
{
	...
	for (int i = 0, j = 0; static_cast<size_t>(i) < m_vecItem.size(); ++i)
	{
