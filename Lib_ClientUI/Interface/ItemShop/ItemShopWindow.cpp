...
CItemShopWindow::CItemShopWindow()
	: m_pListTextBox(NULL), m_pListScrollBar(NULL), m_pButtonPurchase(NULL), m_pButtonPrev(NULL), m_pButtonNext(NULL), m_pTextPage(NULL), m_pTextStaticBattlePoint(NULL), m_pTextStaticEPoint(NULL), m_pTextCharBattlePoint(NULL), m_pTextCharEPoint(NULL), m_pBuyConfirm(NULL), m_pBuyConfirmText(NULL), m_nSelectIndexType(-1), m_nCurrentPage(1), m_sSelectItem(NATIVEID_NULL()), m_sCrow(NATIVEID_NULL()), m_wShopType(0), m_nMaxPage(1), bItemShopConfirm(true), m_strItemNum(""), m_pMidTitleText(NULL), m_pLeftTitleText(NULL), m_pLeftCharTitleText(NULL)
...
void CItemShopWindow::SelectType(DWORD dwType)
{
		 ...
		 iter != m_mapItem.end(); ++iter)
	{
		...
		if (dwType != 0)
		{
			if (shop_item.wItemCtg != dwType)
				continue;
			if (shop_item.wItemCurrency <= 0 || shop_item.wItemCurrency > 2)
				continue;
		}
...
void CItemShopWindow::ShowItem(int nPage)
{
	...
	{
		if (static_cast<size_t>(i) >= m_vecItem.size())
			continue;
