...
void CGlobalRankingPageSlot::TranslateUIMessage(UIGUID ControlID, DWORD dwMsg)
{
	...
	case CLASS_DUMMY:
	{
		if (CHECK_MOUSE_IN(dwMsg))
		{
			...
			case 4:
			{
				CString strTemp = (char*)ID2GAMEWORD("CHAR_CLASS_NAME", CharClassToIndex((EMCHARCLASS)m_sTopLevel.nChaClass));
				CInnerInterface::GetInstance().InfoDisplayShowCommonSingleLine(strTemp, NS_UITEXTCOLOR::WHITE);
			}
			break;
			}
	...
	case SCHOOL_DUMMY:
	{
		if (CHECK_MOUSE_IN(dwMsg))
		{
			...
			case 4:
			{
				CString strTemp = (char*)ID2GAMEWORD("ACADEMY_NAME", m_sTopLevel.wChaSchool);
				CInnerInterface::GetInstance().InfoDisplayShowCommonSingleLine(strTemp, NS_UITEXTCOLOR::WHITE);
			}
			break;
			}
	...
	case GUILD_DUMMY:
	{
		if (CHECK_MOUSE_IN(dwMsg))
		{
			...
			case 4:
			{
				BOOL bGuild = m_sTopLevel.wGuNum ? TRUE : FALSE;
				if (bGuild)
					CInnerInterface::GetInstance().InfoDisplayShowCommonSingleLine(m_sTopLevel.szGuName, NS_UITEXTCOLOR::WHITE);
			}
			/*Top Level*/
			break;
			}
...
HRESULT CGlobalRankingPageSlot::Render(LPDIRECT3DDEVICEQ pd3dDevice)
{
	...
	{
		BOOL bGuild = m_sTopLevel.wGuNum ? TRUE : FALSE;
		if (bGuild && m_sTopLevel.wGuNum > 0)
		{
			m_pGuildImage->SetVisibleSingle(TRUE);
			const int nSERVER					= GLGaeaClient::GetInstance().GetCharacter()->m_dwServerID;
			const DxClubMan::DXDATA& sMarkData	= DxClubMan::GetInstance().GetClubData(pd3dDevice, nSERVER, m_sTopLevel.wGuNum, m_sTopLevel.wGuMarkVer);
...
/*Top Level*/
void CGlobalRankingPageSlot::SetRankingLevel(STOP_RANK_LEVEL sTopLevel)
{
	m_sTopLevel = sTopLevel;

	m_pText_[1]->SetOneLineText(m_sTopLevel.szChaName);

	CString strLevel;
	strLevel.Format("%d", m_sTopLevel.wChaLevel);
	m_pText_[2]->SetOneLineText(strLevel);

	if (m_sTopLevel.bChaOnline)	m_pText_[3]->SetOneLineText("Online", NS_UITEXTCOLOR::BRIGHTGREEN);
	else						m_pText_[3]->SetOneLineText("Offline", NS_UITEXTCOLOR::DISABLE);

	CString strMoney = NS_UITEXTCONTROL::MAKE_MONEY_FORMAT( m_sTopLevel.llnExp, 3, ",");
	m_pText_[4]->SetOneLineText(strMoney.GetString(), NS_UITEXTCOLOR::GOLD);

	CString strClass;
	switch (m_sTopLevel.nChaClass)
	{
	case 1:		strClass = "BR_M.png"; break;
	case 2:		strClass = "SM_M.png"; break;
	case 4:		strClass = "AR_W.png"; break;
