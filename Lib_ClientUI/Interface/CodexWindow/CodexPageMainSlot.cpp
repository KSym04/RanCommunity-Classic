...
CCodexPageMainSlot::CCodexPageMainSlot()
	: m_pStatusComplete(NULL), m_pProgressComplete(NULL), m_pTextComplete(NULL), m_pTextTitle(NULL), m_pTextDesc(NULL), m_pTextProgress(NULL), m_pImageBadge(NULL), m_pImagePoint(NULL), m_pTextBadge(NULL), m_pTextPoint(NULL), m_fUpdateTime(0.0f), m_dwCodexID(UINT_MAX)

	  ,
	  m_pItemImage0(NULL), m_pItemImage1(NULL), m_pItemImage2(NULL), m_pItemImage3(NULL), m_pItemImage4(NULL)

	  ,
	  m_sidItem0(NATIVEID_NULL()), m_sidItem1(NATIVEID_NULL()), m_sidItem2(NATIVEID_NULL()), m_sidItem3(NATIVEID_NULL()), m_sidItem4(NATIVEID_NULL())

	  ,
	  bItem0(false), bItem1(false), bItem2(false), bItem3(false), bItem4(false)

	  ,
	  m_wGradeItem0(0), m_wGradeItem1(0), m_wGradeItem2(0), m_wGradeItem3(0), m_wGradeItem4(0)
...
void CCodexPageMainSlot::TranslateUIMessage(UIGUID ControlID, DWORD dwMsg)
{
	...
	case ITEM_SLOT_IMAGE0:
	{
		if (CHECK_MOUSE_IN(dwMsg))
		{
			...
			if (pitem_data && pitem_data->ISPILE())
			...
			case SUIT_FOOT:
			{
				sitemcustom.cDEFENSE = static_cast<BYTE>(m_wGradeItem0);
			}
			break;

			case SUIT_HANDHELD:
			{
				sitemcustom.cDAMAGE = static_cast<BYTE>(m_wGradeItem0);
			}
			break;
			};
	...
	case ITEM_SLOT_IMAGE1:
	{
		if (CHECK_MOUSE_IN(dwMsg))
		{
			...
			case SUIT_FOOT:
			{
				sitemcustom.cDEFENSE = static_cast<BYTE>(m_wGradeItem1);
			}
			break;

			case SUIT_HANDHELD:
			{
				sitemcustom.cDAMAGE = static_cast<BYTE>(m_wGradeItem1);
			}
			break;
			};
	...
	case ITEM_SLOT_IMAGE2:
	{
		if (CHECK_MOUSE_IN(dwMsg))
		{
			...
			case SUIT_FOOT:
			{
				sitemcustom.cDEFENSE = static_cast<BYTE>(m_wGradeItem2);
			}
			break;

			case SUIT_HANDHELD:
			{
				sitemcustom.cDAMAGE = static_cast<BYTE>(m_wGradeItem2);
			}
			break;
			};
	...
	case ITEM_SLOT_IMAGE3:
	{
		if (CHECK_MOUSE_IN(dwMsg))
		{
			...
			case SUIT_FOOT:
			{
				sitemcustom.cDEFENSE = static_cast<BYTE>(m_wGradeItem3);
			}
			break;

			case SUIT_HANDHELD:
			{
				sitemcustom.cDAMAGE = static_cast<BYTE>(m_wGradeItem3);
			}
			break;
			};
	...
	case ITEM_SLOT_IMAGE4:
	{
		if (CHECK_MOUSE_IN(dwMsg))
		{
			...
			case SUIT_FOOT:
			{
				sitemcustom.cDEFENSE = static_cast<BYTE>(m_wGradeItem4);
			}
			break;

			case SUIT_HANDHELD:
			{
				sitemcustom.cDAMAGE = static_cast<BYTE>(m_wGradeItem4);
			}
			break;
			};
...
void CCodexPageMainSlot::UpdateInfo()
{
	...
	if (pcodex_data)
	{
		...
		if (pItemData0)
		{
			...
			WORD dwGrade = pcodex_data->wItemGrade1;
			WORD dwQuantity = pcodex_data->wProgressMobKill;
		...
		if (pItemData1)
		{
			...
			WORD dwGrade = pcodex_data->wItemGrade2;
			WORD dwQuantity = pcodex_data->wProgressMapKill;
		...
		if (pItemData2)
		{
			...
			WORD dwGrade = pcodex_data->wItemGrade3;
			WORD dwQuantity = pcodex_data->wProgressMapReach;
		...
		if (pItemData3)
		{
			...
			WORD dwGrade = pcodex_data->wItemGrade4;
			WORD dwQuantity = pcodex_data->wProgressItemGet;
		...
		if (pItemData4)
		{
			...
			WORD dwGrade = pcodex_data->wItemGrade5;
			WORD dwQuantity = pcodex_data->wProgressItemUse;
		...
		if (pcodex_data->bRewardBadge && pcodex_data->strBadgeString.size())
		{
			if (m_pImageBadge)
				m_pImageBadge->SetVisibleSingle(FALSE);
			if (m_pTextBadge)
				m_pTextBadge->AddText(pcodex_data->strBadgeString.c_str(), NS_UITEXTCOLOR::WHITE);
		}
		...
		if (pchar_codex_data_done)
		{
			...
			if (m_pTextProgress)
				m_pTextProgress->AddText(ID2GAMEWORD("RAN_SPEC_CODEX_PROGRESS_TEXT", 1), NS_UITEXTCOLOR::GREENYELLOW);
		...
		if (pchar_codex_data_prog)
		{
			...
			if (m_pTextProgress)
				m_pTextProgress->AddText("", NS_UITEXTCOLOR::WHITE);
