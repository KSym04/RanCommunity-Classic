...
CItemImage::CItemImage()
	: m_pItemImageBack(NULL), m_pItemImage(NULL), m_pd3dDevice(NULL), m_sICONINDEXBACK(NATIVEID_NULL()), m_sICONINDEX(NATIVEID_NULL()), m_TotElapsedTime(0.0f), m_bItemUsed(false), m_bItemEnd(false), m_bTwinkle(true), m_pItemProcess(NULL), m_sNativeID(false), m_pTextBoxUpLeft(NULL), m_pTextBoxUpRight(NULL), m_pTextBoxDownLeft(NULL), m_pTextBoxDownRight(NULL), m_pItemImageCheck(NULL) /*6-2-24, collection system - CNDev*/
	  ,
...
void CItemImage::Update(int x, int y, BYTE LB, BYTE MB, BYTE RB, int nScroll, float fElapsedTime, BOOL bFirstControl)
{
	...
	if (m_bItemUsed && m_bItemEnd)
	{
		...
		if (m_TotElapsedTime < 0.3f)
			m_pItemProcess->SetPercent(0);
		else if (m_TotElapsedTime < 0.6f)
			m_pItemProcess->SetPercent(1);
		else if (m_TotElapsedTime < 0.9f)
			m_pItemProcess->SetPercent(0);
		else if (m_TotElapsedTime < 1.2f)
			m_pItemProcess->SetPercent(1);
		else
		{
			m_pItemProcess->SetPercent(0);
	...
	float fCurTime = fCoolTime - static_cast<float>(tCurTime - tUseTime);
...
void CItemImage::SetBackItem(SNATIVEID sICONINDEX, const char *szTexture)
{
	if (m_pItemImageBack)
	{
		if ((sICONINDEX == m_sICONINDEXBACK) && (szTexture == m_pItemImageBack->GetTextureName()))
			return;
...
{
	if (m_pTextBoxUpLeft)
		m_pTextBoxUpLeft->SetText(strText.GetString(), dwColor);
}

void CItemImage::SetTextUpRight(CString strText, DWORD dwColor)
{
	if (m_pTextBoxUpRight)
		m_pTextBoxUpRight->SetText(strText.GetString(), dwColor);
}

void CItemImage::SetTextDownLeft(CString strText, DWORD dwColor)
{
	if (m_pTextBoxDownLeft)
		m_pTextBoxDownLeft->SetText(strText.GetString(), dwColor);
}

void CItemImage::SetTextDownRight(CString strText, DWORD dwColor)
{
	if (m_pTextBoxDownRight)
		m_pTextBoxDownRight->SetText(strText.GetString(), dwColor);
}
