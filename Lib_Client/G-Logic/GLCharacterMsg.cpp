...
void GLCharacter::MsgProcess(NET_MSG_GENERIC *nmg)
{
	...
	{
		GLMSG::SNETPC_REQ_GLOBAL_RANKING_LEVEL_AGT *pNetMsg = (GLMSG::SNETPC_REQ_GLOBAL_RANKING_LEVEL_AGT *)nmg;

		if (pNetMsg->emFB > EM_GLOBAL_RANKING_FB_DELAY)
			for (int i = 0; i < 10; i++)
				if (pNetMsg->sTopLevel_[i].dwChaNum > 0)
					CInnerInterface::GetInstance().GetGlobalRankingWindow()->SetRankingLevel(pNetMsg->wPart, pNetMsg->sTopLevel_[i], i);
	}
	break;
	...
	case NET_MSG_GCTRL_ACTION_BRD:
	{
		...
		if (pNetMsg->emAction == GLAT_FALLING && m_Action >= GLAT_FALLING)
			break;
...
*/
	...
	case NET_MSG_GCTRL_PUTON_UPDATE:
	{
		...
		if (pNetMsg->emSlot != SLOT_HOLD)
		{
			...
			if (wACP > 0)
				CInnerInterface::GetInstance().PrintMsgTextDlg(NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("ITEMPUTON_ACCEPT_SP"), wACP);
	...
	case NET_MSG_GCTRL_UPDATE_CHA_PROMISE_POINTS: // gacha system, jhoniex 7/7/2024
	{
		...
		m_wGachaPromisePoint = static_cast<WORD>(pNetMsg->dwVal);
	}
	break;
...
// 경험치회복_정의_Npc
	...
	case NET_MSG_GCTRL_PLAYERKILLING_ADD:
	{
		...
		if (bADD && !pNetMsg->bBAD && !pLand->m_bPVPCaptureTheFlagMap)
		{
			if (pNetMsg->bClubBattle)
			{
				...
				CInnerInterface::GetInstance().PrintConsoleTextDlg(ID2GAMEINTEXT("PK_PLAYHOSTILE_ADD"), pNetMsg->szName);
		}
	}
	break;
		...
		case EMVEHICLE_REQ_GIVE_BATTERY_FB_OK:
		{
			...
			if (pItem && pVehicleItem)
			{
				...
				if (m_sVehicle.m_dwGUID == pNetMsg->dwVehicleID)
					m_sVehicle.IncreaseFull(pItem->sDrugOp.wCureVolume, pItem->sDrugOp.bRatio);
	...
	case NET_MSG_REQ_GATHERING_FB:
	{
		...
		{
			CInnerInterface::GetInstance().PrintMsgText(NS_UITEXTCOLOR::PALEGREEN, ID2GAMEINTEXT("EMREQ_GATHER_FB_OK"));
		...
		{
			CInnerInterface::GetInstance().PrintMsgText(NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMREQ_GATHER_FB_FAIL"));
		}
		break;
		...
		{
			CInnerInterface::GetInstance().PrintMsgText(NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMREQ_GATHER_FB_NOCROW"));
		}
		break;
		case EMREQ_GATHER_FB_USE:
		{
			CInnerInterface::GetInstance().PrintMsgText(NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMREQ_GATHER_FB_USE"));
		}
		break;
		case EMREQ_GATHER_FB_GATHERING:
		{
			CInnerInterface::GetInstance().PrintMsgText(NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMREQ_GATHER_FB_GATHERING"));
		}
		break;
		}
	}
	...
	case NET_MSG_REQ_GATHERING_RESULT:
	{
		...
		{
			CInnerInterface::GetInstance().PrintMsgText(NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMREQ_GATHER_RESULT_SUCCESS_EX"));
		...
		{
			CInnerInterface::GetInstance().PrintMsgText(NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMREQ_GATHER_RESULT_FAIL"));
		...
		{
			CInnerInterface::GetInstance().PrintMsgText(NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMREQ_GATHER_RESULT_ITEMFAIL"));
	...
	case NET_MSG_GCTRL_VEHICLE_BOOSTER_STATE_START:
	{
		...
		if (!m_bBoosterStart && IsSTATE(EM_ACT_VEHICLE_BOOSTER))
	...
	case NET_MSG_GCTRL_PK_STREAK:
	{
		...
		if (kills >= PK_STREAK_SIZE)
			kills = PK_STREAK_SIZE;
		...
		case EMREQ_GM_GET_CROW_TIME_FB_OK:
		{
			...
			if (pCrowData && pMapNode)
			{
				...
				else
				{
					...
					{
						std::string strDay = "";
						if (pNetMsg->bDayOfWeek[0])
							strDay += "Sunday,";
						if (pNetMsg->bDayOfWeek[1])
							strDay += "Monday,";
						if (pNetMsg->bDayOfWeek[2])
							strDay += "Tuesday,";
						if (pNetMsg->bDayOfWeek[3])
							strDay += "Wednesday,";
						if (pNetMsg->bDayOfWeek[4])
							strDay += "Thursday,";
						if (pNetMsg->bDayOfWeek[5])
							strDay += "Friday,";
						if (pNetMsg->bDayOfWeek[6])
							strDay += "Saturday,";
