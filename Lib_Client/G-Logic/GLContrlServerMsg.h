...
namespace GLMSG
{
	...
	/* security 2025 */
	struct SNET_DETECT_CHEAT_C2A : public NET_MSG_GENERIC
	{
		DWORD	dwCharID;
		char	szName[CHAR_SZNAME];

		SNET_DETECT_CHEAT_C2A()
			: dwCharID(0)
		{
			memset(szName, 0, sizeof(char)*CHAR_SZNAME);

			dwSize = sizeof(*this);
			nType = NET_DETECT_CHEAT_C2A;
			GASSERT(dwSize <= NET_DATA_BUFSIZE);
		}
	};

	struct SNET_DETECT_CHEAT_A2C : public NET_MSG_GENERIC
	{
		DWORD	dwCharID;
		char	szName[CHAR_SZNAME];

		SNET_DETECT_CHEAT_A2C()
			: dwCharID(0)
		{
			memset(szName, 0, sizeof(char)*CHAR_SZNAME);

			dwSize = sizeof(*this);
			nType = NET_DETECT_CHEAT_A2C;
			GASSERT(dwSize <= NET_DATA_BUFSIZE);
		}
	};
