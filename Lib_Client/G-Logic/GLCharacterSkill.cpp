...
void GLCharacter::SkillReaction(const STARGETID &sTargetID, const DWORD dwMR, const bool bcontinue, BOOL &bMove, D3DXVECTOR3 &vMoveTo)
{
	...
	{
		D3DXVECTOR3 vPos1 = GetPosition();
		vPos1.y += 15.0f;
		D3DXVECTOR3 vPos2 = vTarPos;
		vPos2.y += 15.0f;
	...
	if (emSkillCheck != EMSKILL_OK && emSkillCheck != EMSKILL_NOTSP)
	{
		if (dwMR & (DXKEY_UP))
		{
			...
			case EMSKILL_NOTITEM:
			{
				...
				{
					std::string strITEM;
					if (pSkill->m_sBASIC.emUSE_LITEM != SKILLATT_NOCARE)
						strITEM = COMMENT::SKILLATTACK[pSkill->m_sBASIC.emUSE_LITEM].c_str();
					if (pSkill->m_sBASIC.emUSE_RITEM != SKILLATT_NOCARE)
						strITEM = COMMENT::SKILLATTACK[pSkill->m_sBASIC.emUSE_RITEM].c_str();

					if (strITEM.empty())
						CInnerInterface::GetInstance().PrintMsgTextDlg(NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("SKILL_NOTITEM"));
	...
	if (!bOK)
	{
		...
		if (dwMR & (DXKEY_UP))
			CInnerInterface::GetInstance().PrintMsgText(NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("NOTARGET_SKILL"));
