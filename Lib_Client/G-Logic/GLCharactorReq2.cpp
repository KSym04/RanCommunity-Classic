...
HRESULT GLCharacter::ReqInvenToWear(WORD wPosX, WORD wPosY)
{
	...
	{
	case SUIT_HEADGEAR:
		emUseSlot = SLOT_HEADGEAR;
		break;
	case SUIT_UPPER:
		emUseSlot = SLOT_UPPER;
		break;
	case SUIT_LOWER:
		emUseSlot = SLOT_LOWER;
		break;
	case SUIT_HAND:
		emUseSlot = SLOT_HAND;
		break;
	...
	case SUIT_BELT:
		...
		break;
	case SUIT_EARRING:
		emUseSlot = SLOT_EARRING;
		break;
	case SUIT_DECORATION:
		emUseSlot = SLOT_DECORATION;
		break;
	case SUIT_WING:
		emUseSlot = SLOT_WING;
		break;
	...
	if (emUseSlot == SLOT_TSIZE)
		return E_FAIL;

	if (!CHECKSLOT_ITEM(pITEM->sBasicOp.sNativeID, emUseSlot))
		return E_FAIL;
...
HRESULT GLCharacter::ReqRandomOptionChange(WORD wPosX, WORD wPosY)
{
	...
	{
		if (ptarget_inven_item->sItemCustom.cOptTYPE1 == (BYTE)pitem_data_hold->sRvCard.emOption && ptarget_inven_item->sItemCustom.nOptVALUE1 >= (short)pitem_data_hold->sRvCard.wValue)
		{
		...
		if (ptarget_inven_item->sItemCustom.cOptTYPE2 == (BYTE)pitem_data_hold->sRvCard.emOption && ptarget_inven_item->sItemCustom.nOptVALUE2 >= (short)pitem_data_hold->sRvCard.wValue)
		{
			CInnerInterface::GetInstance().PrintMsgText(NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EM_RANDOM_OPTION_CHANGE_TARGET_OPTION_ALREADY_SET"));
			return S_FALSE;
		}

		if (ptarget_inven_item->sItemCustom.cOptTYPE3 == (BYTE)pitem_data_hold->sRvCard.emOption && ptarget_inven_item->sItemCustom.nOptVALUE3 >= (short)pitem_data_hold->sRvCard.wValue)
		{
			CInnerInterface::GetInstance().PrintMsgText(NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EM_RANDOM_OPTION_CHANGE_TARGET_OPTION_ALREADY_SET"));
			return S_FALSE;
		}

		if (ptarget_inven_item->sItemCustom.cOptTYPE4 == (BYTE)pitem_data_hold->sRvCard.emOption && ptarget_inven_item->sItemCustom.nOptVALUE4 >= (short)pitem_data_hold->sRvCard.wValue)
		{
			CInnerInterface::GetInstance().PrintMsgText(NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EM_RANDOM_OPTION_CHANGE_TARGET_OPTION_ALREADY_SET"));
			return S_FALSE;
		}
	...
	else
	{
		...
		if (ptarget_inven_item->sItemCustom.cOptTYPE1 == EMR_OPT_NULL)
			bfound_free = TRUE;
		if (ptarget_inven_item->sItemCustom.cOptTYPE2 == EMR_OPT_NULL)
			bfound_free = TRUE;
		if (ptarget_inven_item->sItemCustom.cOptTYPE3 == EMR_OPT_NULL)
			bfound_free = TRUE;
		if (ptarget_inven_item->sItemCustom.cOptTYPE4 == EMR_OPT_NULL)
			bfound_free = TRUE;
...
BOOL GLCharacter::InvenUseToPutOn(EMSLOT emSlot)
{
	...
	if (!SIMPLE_CHECK_ITEM(pitem_data_hold->sBasicOp.sNativeID))
		return TRUE;
	if (CheckCoolTime(pitem_data_hold->sBasicOp.sNativeID))
		return TRUE;
...
{
	if (m_emSlotItemColor >= SLOT_TSIZE)
		return;
...
const SITEMCUSTOM GLCharacter::GetItemTransferItemOld()
{
	...
	if (!m_sItemTransferPosOld.VALID())
		return sItemCustom;
...
const SITEMCUSTOM GLCharacter::GetItemTransferItemNew()
{
	SITEMCUSTOM sItemCustom;
	sItemCustom.sNativeID = NATIVEID_NULL();
	if (!m_sItemTransferPosNew.VALID())
		return sItemCustom;
...
{
	if (CInnerInterface::GetInstance().IsVisibleGroup(ITEM_TRANSFER_INVEN_WINDOW) || CInnerInterface::GetInstance().IsVisibleGroup(ITEM_TRANSFER_WINDOW))
		return true;
	return false;
}
...
const SITEMCUSTOM GLCharacter::GetItemRandomOptionRebuildItem()
{
	SITEMCUSTOM sItemCustom;
	sItemCustom.sNativeID = NATIVEID_NULL();
	if (!m_sItemRandomOptionRebuildItem.VALID())
		return sItemCustom;
...
{
	if (CInnerInterface::GetInstance().IsVisibleGroup(ITEM_RANDOM_OPTION_REBUILD_INVENTORY_WINDOW) || CInnerInterface::GetInstance().IsVisibleGroup(ITEM_RANDOM_OPTION_REBUILD_WINDOW))
		return true;
	return false;
}
...
{
	WORD num(0);
	if (m_bItemRandomOptionRebuildOptionLock0)
		num++;
	if (m_bItemRandomOptionRebuildOptionLock1)
		num++;
	if (m_bItemRandomOptionRebuildOptionLock2)
		num++;
	if (m_bItemRandomOptionRebuildOptionLock3)
		num++;
...
void GLCharacter::SetRandomOptionRebuildLock(WORD windex, BOOL bset)
{
	...
	if (pitem->sBasicOp.emItemType != ITEM_SEALED_CARD)
		return;

	if (bset && GetItemRandomOptionRebuildLockNum() >= pitem->sSuitOp.wReModelNum)
		return;
...
void GLCharacter::do_away_gesture()
{
	if (!m_pSkinChar)
		return;
	if (GLCONST_CHAR::nAWAY_GESTURE_NUMBER >= AN_SUB_00_SIZE)
		return;
	if (m_pSkinChar->GETCURSTYPE() == GLCONST_CHAR::nAWAY_GESTURE_NUMBER)
		return;
...
HRESULT GLCharacter::item_box_specific_open_window(WORD wPosX, WORD wPosY)
{
	...
	if (!SIMPLE_CHECK_ITEM(pitem_data->sBasicOp.sNativeID))
		return E_FAIL;
...
HRESULT GLCharacter::item_box_specific_select_item(SNATIVEID item_id, int selected_index, SNATIVEID selected_item_id)
{
	...
	if (!SIMPLE_CHECK_ITEM(pitem_data_main->sBasicOp.sNativeID))
		return E_FAIL;
...
void GLCharacter::req_lunchbox_remove(SNATIVEID skill_id)
{
	...
	if (!RPARAM::allow_lunchbox_remove)
		return;
...
{
	if (CInnerInterface::GetInstance().IsVisibleGroup(CHAOS_MACHINE_INVEN_WINDOW) || CInnerInterface::GetInstance().IsVisibleGroup(CHAOS_MACHINE_WINDOW))
		return true;
	return false;
}
...
const SITEMCUSTOM GLCharacter::GetChaosMachineItem()
{
	...
	if (!m_sChaosMachineItemPos.VALID())
		return sItemCustom;
...
HRESULT GLCharacter::ReqDisguiseExtend(WORD wPosX, WORD wPosY)
{
	...
	if (pHold->sBasicOp.emItemType != ITEM_CARD_COSTUME_EXTENDER)
		return S_FALSE;
