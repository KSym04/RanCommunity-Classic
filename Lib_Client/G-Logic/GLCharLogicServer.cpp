									   ...
									   const WORD _wSchool, const WORD wHair, const WORD wFace, const WORD _wHairColor, const WORD _wSex, const float _fScaleRange)
{
	...
	if (emCIndex >= GLCI_NUM_8CLASS)
		emCIndex = GLCI_BRAWLER_M;
	...
	if (cCONST.dwHAIRNUM <= m_wHair)
		m_wHair = 0;
	if (cCONST.dwHEADNUM <= m_wFace)
		m_wFace = 0;
...
void GLCHARLOGIC_SERVER::SUM_ADDITION(float fCONFT_POINT_RATE)
{
	...
	if (m_sHP.wMax < m_dwHPIncrease || m_sHP.wMax > USHRT_MAX)
		m_sHP.wMax = USHRT_MAX;
	// m_sHP.LIMIT();
	...
	if (ISLONGRANGE_ARMS())
		m_gdDAMAGE_PHYSIC.VAR_PARAM(m_wSUM_SA); //	장거리 공격.
	else
		m_gdDAMAGE_PHYSIC.VAR_PARAM(m_wSUM_PA); //	근접 공격.
...
void GLCHARLOGIC_SERVER::SUM_ITEM()
{
	...
	for (int i = 0; i < SLOT_NSIZE_S_2; i++)
	{
		...
		case EMVAR_MOVE_SPEED:
			// 나중에 합산으로 변경
			if (emSLOT != SLOT_VEHICLE)
				m_sSUMITEM.fInc_MoveSpeed += sItemCustom.GETMOVESPEED();
			else if (m_bVehicle)
				m_sSUMITEM.fInc_MoveSpeed += m_fVehicleSpeedVol + sItemCustom.GETMOVESPEED();
			break;
		case EMVAR_ATTACK_SPEED:
...
void GLCHARLOGIC_SERVER::SUM_PASSIVE()
{
	...
	for (; iter != iter_end; ++iter)
	{
		...
		const bool bHiddenWeapon = (pSkill->m_sLEARN.bHiddenWeapon != FALSE);
...
BOOL GLCHARLOGIC_SERVER::CHECKHIT(const STARGETID &cTargetID, const GLLandMan *pLandMan, const BOOL bLowSP)
{
	...
	if (cTargetID.dwID == EMTARGET_NULL)
		return FALSE;
	...
	if (bLowSP)
		nHitRate = int(nHitRate * (1.0f - GLCONST_CHAR::fLOWSP_HIT_DROP));
	...
	const int dwDivCount)
{
	...
	if (ndxLvl > 0)
		nExtFORCE = int(RANDOM_POS * ndxLvl / 10);
	...
	if (nCrushingBlow > static_cast<int>(GLCONST_CHAR::dwCRUSHING_BLOW_MAX))
		nCrushingBlow = static_cast<int>(GLCONST_CHAR::dwCRUSHING_BLOW_MAX);
	...
	if (nNetDAMAGE > 0)
		rResultDAMAGE = int(nDAMAGE_OLD - nDEFENSE);
	else
		rResultDAMAGE = int((nDAMAGE_OLD * GLCONST_CHAR::fLOW_SEED_DAMAGE) * RANDOM_POS);
	...
	{
		if (bCritical)
			rResultDAMAGE = int(rResultDAMAGE * ncritical_hit_damage / 100);
		if (bCrushingBlow)
			rResultDAMAGE = int(rResultDAMAGE * ncrushing_blow_damage / 100);
	}
	...
	bShock = (GLOGICEX::CHECKSHOCK(GETLEVEL(), nLEVEL, rResultDAMAGE, bCritical) != FALSE);
	...
	const int dwDivCount)
{
	...
	int ndxLvl = nLEVEL - GETLEVEL();
	if (ndxLvl > 0)
		nExtFORCE = int(RANDOM_POS * ndxLvl * 0.1f);
...
void GLCHARLOGIC_SERVER::UPDATE_DATA(float fTime, float fElapsedTime, BOOL bClient, float fCONFT_POINT_RATE)
{
	...
	for (int i = 0; i < SKILLFACT_SIZE; ++i)
	{
		...
		case SKILL::EMFOR_RESIST:
			...
			break;
		case SKILL::EMFOR_SUMMONTIME:
			m_nSummonTime += (int)sSKEFF.fMVAR;
			break; /*summon time, Juver, 2017/12/12 */
		};
		...
		for (int nImpact = 0; nImpact < SKILL::MAX_IMPACT; ++nImpact)
		{
			...
			case EMIMPACTA_AVOIDRATE:
				...
				break;
			case EMIMPACTA_DAMAGE:
				m_gdDAMAGE_SKILL.VAR_PARAM(int(sSKEFF.sImpacts[nImpact].fADDON_VAR));
				break;
			case EMIMPACTA_DEFENSE:
			...
			case EMIMPACTA_PA:
				nSUM_PA += int(sSKEFF.sImpacts[nImpact].fADDON_VAR);
				break;
			case EMIMPACTA_SA:
				nSUM_SA += int(sSKEFF.sImpacts[nImpact].fADDON_VAR);
				break;
			case EMIMPACTA_MA:
				nSUM_MA += int(sSKEFF.sImpacts[nImpact].fADDON_VAR);
				break;
			case EMIMPACTA_HP_RATE:
				m_fHP_RATE += sSKEFF.sImpacts[nImpact].fADDON_VAR;
				break;
			case EMIMPACTA_MP_RATE:
				m_fMP_RATE += sSKEFF.sImpacts[nImpact].fADDON_VAR;
				break;
			case EMIMPACTA_SP_RATE:
				m_fSP_RATE += sSKEFF.sImpacts[nImpact].fADDON_VAR;
				break;
			case EMIMPACTA_RESIST:
				m_sSUMRESIST_SKILL += int(sSKEFF.sImpacts[nImpact].fADDON_VAR);
				break;
			};
		}
		...
		for (int nSpec = 0; nSpec < SKILL::MAX_SPEC; ++nSpec)
		{
			...
			case EMSPECA_PROHIBIT_POTION:
				m_bProhibitPotion = true;
				break;

				/*prohibit skill logic, Juver, 2017/06/06 */
			case EMSPECA_PROHIBIT_SKILL:
				m_bProhibitSkill = true;
				break;
			...
			case EMSPECA_DEFENSE_SKILL_ACTIVE:
			{
				...
				m_sDefenseSkill.m_wLevel = static_cast<WORD>(sSKEFF.sSpecs[nSpec].dwSPECFLAG);
	...
	if (m_sQITEMFACT.IsACTIVE())
	{
		...
		case QUESTION_EXP_UP:
			...
			break;
		case QUESTION_LUCKY:
			m_fGEN_RATE += (m_sQITEMFACT.wParam1 / 100.0f);
			break;
		case QUESTION_SPEED_UP_M:
			m_fSKILL_MOVE += (m_sQITEMFACT.wParam1 / 100.0f);
			break;
	...
	for (int i = 0; i < EMLANDEFFECT_MULTI; i++)
	{
		...
		case EMLANDEFFECT_RESIST_RATE: // 저항 수치
			m_sSUM_PASSIVE.m_sSUMRESIST.nFire += static_cast<short>(landEffect.fValue);
			m_sSUM_PASSIVE.m_sSUMRESIST.nIce += static_cast<short>(landEffect.fValue);
			m_sSUM_PASSIVE.m_sSUMRESIST.nElectric += static_cast<short>(landEffect.fValue);
			m_sSUM_PASSIVE.m_sSUMRESIST.nPoison += static_cast<short>(landEffect.fValue);
			m_sSUM_PASSIVE.m_sSUMRESIST.nSpirit += static_cast<short>(landEffect.fValue);
			break;
	...
	for (int i = 0; i < FITEMFACT_SIZE; ++i)
	{
		...
		if (sFITEMEFF.sNATIVEID == NATIVEID_NULL())
			continue;
		...
		case SKILL::EMFOR_DEFENSE:
			m_nDEFENSE_SKILL += (int)sFITEMEFF.fMVAR;
			break;
		case SKILL::EMFOR_HITRATE:
			m_nSUM_HIT += int(sFITEMEFF.fMVAR);
			break;
		case SKILL::EMFOR_AVOIDRATE:
			m_nSUM_AVOID += int(sFITEMEFF.fMVAR);
			break;
		case SKILL::EMFOR_VARDAMAGE:
			m_gdDAMAGE_SKILL.VAR_PARAM(int(sFITEMEFF.fMVAR));
			break;
		case SKILL::EMFOR_VARDEFENSE:
			m_nDEFENSE_SKILL += int(sFITEMEFF.fMVAR);
			break;
		case SKILL::EMFOR_PA:
			nSUM_PA += int(sFITEMEFF.fMVAR);
			break;
		case SKILL::EMFOR_SA:
			nSUM_SA += int(sFITEMEFF.fMVAR);
			break;
		case SKILL::EMFOR_MA:
			nSUM_MA += int(sFITEMEFF.fMVAR);
			break;
		case SKILL::EMFOR_HP_RATE:
			m_fHP_RATE += sFITEMEFF.fMVAR;
			break;
		case SKILL::EMFOR_MP_RATE:
			m_fMP_RATE += sFITEMEFF.fMVAR;
			break;
		case SKILL::EMFOR_SP_RATE:
			m_fSP_RATE += sFITEMEFF.fMVAR;
			break;
		case SKILL::EMFOR_RESIST:
			m_sSUMRESIST_SKILL += int(sFITEMEFF.fMVAR);
			break;
		case SKILL::EMFOR_SUMMONTIME:
			m_nSummonTime += (int)sFITEMEFF.fMVAR;
			break; /*summon time, Juver, 2017/12/12 */
		};
		...
		for (int nImpact = 0; nImpact < SKILL::MAX_IMPACT; ++nImpact)
		{
			...
			{
			case EMIMPACTA_HITRATE:
				m_nSUM_HIT += int(sFITEMEFF.sImpacts[nImpact].fADDON_VAR);
				break;
			case EMIMPACTA_AVOIDRATE:
				m_nSUM_AVOID += int(sFITEMEFF.sImpacts[nImpact].fADDON_VAR);
				break;
			case EMIMPACTA_DAMAGE:
				m_gdDAMAGE_SKILL.VAR_PARAM(int(sFITEMEFF.sImpacts[nImpact].fADDON_VAR));
				break;
			case EMIMPACTA_DEFENSE:
				m_nDEFENSE_SKILL += int(sFITEMEFF.sImpacts[nImpact].fADDON_VAR);
				break;
			case EMIMPACTA_VARHP:
			...
			case EMIMPACTA_DAMAGE_RATE:
				m_fDamageRate += sFITEMEFF.sImpacts[nImpact].fADDON_VAR;
				break;
			case EMIMPACTA_DEFENSE_RATE:
				m_fDefenseRate += sFITEMEFF.sImpacts[nImpact].fADDON_VAR;
				break;

			case EMIMPACTA_PA:
				nSUM_PA += int(sFITEMEFF.sImpacts[nImpact].fADDON_VAR);
				break;
			case EMIMPACTA_SA:
				nSUM_SA += int(sFITEMEFF.sImpacts[nImpact].fADDON_VAR);
				break;
			case EMIMPACTA_MA:
				nSUM_MA += int(sFITEMEFF.sImpacts[nImpact].fADDON_VAR);
				break;
			case EMIMPACTA_HP_RATE:
				m_fHP_RATE += sFITEMEFF.sImpacts[nImpact].fADDON_VAR;
				break;
			case EMIMPACTA_MP_RATE:
				m_fMP_RATE += sFITEMEFF.sImpacts[nImpact].fADDON_VAR;
				break;
			case EMIMPACTA_SP_RATE:
				m_fSP_RATE += sFITEMEFF.sImpacts[nImpact].fADDON_VAR;
				break;
			case EMIMPACTA_RESIST:
				m_sSUMRESIST_SKILL += int(sFITEMEFF.sImpacts[nImpact].fADDON_VAR);
				break;
			};
		}
		...
		for (int nSpec = 0; nSpec < SKILL::MAX_SPEC; ++nSpec)
		{
			...
			{
			case EMSPECA_NONBLOW:
				m_dwHOLDBLOW = sFITEMEFF.sSpecs[nSpec].dwSPECFLAG;
				break;
			case EMSPECA_PIERCE:
			...
			case EMSPECA_TARRANGE:
				...
				break;
			case EMSPECA_MOVEVELO:
				m_fSKILL_MOVE += sFITEMEFF.sSpecs[nSpec].fSPECVAR1;
				break;

				//	공격 속도 계념이므로 ( 공격 시간을 -0.1 (-10%) 감소 시키기 위해서는 부호 반전을 하여야함. )
			case EMSPECA_ATTACKVELO:
				m_fATTVELO -= sFITEMEFF.sSpecs[nSpec].fSPECVAR1;
				break;
			...
			case EMSPECA_DEFENSE_SKILL_ACTIVE:
			{
				...
				m_sDefenseSkill.m_wLevel = static_cast<WORD>(sFITEMEFF.sSpecs[nSpec].dwSPECFLAG);
	...
	for (int i = 0; i < SYSTEM_BUFF_SIZE; ++i)
	{
		...
		if (ssystem_buff.sNATIVEID == NATIVEID_NULL())
			continue;
		...
		case SKILL::EMFOR_DEFENSE:
			m_nDEFENSE_SKILL += (int)ssystem_buff.fMVAR;
			break;
		case SKILL::EMFOR_HITRATE:
			m_nSUM_HIT += int(ssystem_buff.fMVAR);
			break;
		case SKILL::EMFOR_AVOIDRATE:
			m_nSUM_AVOID += int(ssystem_buff.fMVAR);
			break;
		case SKILL::EMFOR_VARDAMAGE:
			m_gdDAMAGE_SKILL.VAR_PARAM(int(ssystem_buff.fMVAR));
			break;
		case SKILL::EMFOR_VARDEFENSE:
			m_nDEFENSE_SKILL += int(ssystem_buff.fMVAR);
			break;
		case SKILL::EMFOR_PA:
			nSUM_PA += int(ssystem_buff.fMVAR);
			break;
		case SKILL::EMFOR_SA:
			nSUM_SA += int(ssystem_buff.fMVAR);
			break;
		case SKILL::EMFOR_MA:
			nSUM_MA += int(ssystem_buff.fMVAR);
			break;
		case SKILL::EMFOR_HP_RATE:
			m_fHP_RATE += ssystem_buff.fMVAR;
			break;
		case SKILL::EMFOR_MP_RATE:
			m_fMP_RATE += ssystem_buff.fMVAR;
			break;
		case SKILL::EMFOR_SP_RATE:
			m_fSP_RATE += ssystem_buff.fMVAR;
			break;
		case SKILL::EMFOR_RESIST:
			m_sSUMRESIST_SKILL += int(ssystem_buff.fMVAR);
			break;
		case SKILL::EMFOR_SUMMONTIME:
			m_nSummonTime += (int)ssystem_buff.fMVAR;
			break; /*summon time, Juver, 2017/12/12 */
		};
		...
		for (int nImpact = 0; nImpact < SKILL::MAX_IMPACT; ++nImpact)
		{
			...
			{
			case EMIMPACTA_HITRATE:
				m_nSUM_HIT += int(ssystem_buff.sImpacts[nImpact].fADDON_VAR);
				break;
			case EMIMPACTA_AVOIDRATE:
				m_nSUM_AVOID += int(ssystem_buff.sImpacts[nImpact].fADDON_VAR);
				break;
			case EMIMPACTA_DAMAGE:
				m_gdDAMAGE_SKILL.VAR_PARAM(int(ssystem_buff.sImpacts[nImpact].fADDON_VAR));
				break;
			case EMIMPACTA_DEFENSE:
				m_nDEFENSE_SKILL += int(ssystem_buff.sImpacts[nImpact].fADDON_VAR);
				break;
			case EMIMPACTA_VARHP:
			...
			case EMIMPACTA_DAMAGE_RATE:
				m_fDamageRate += ssystem_buff.sImpacts[nImpact].fADDON_VAR;
				break;
			case EMIMPACTA_DEFENSE_RATE:
				m_fDefenseRate += ssystem_buff.sImpacts[nImpact].fADDON_VAR;
				break;

			case EMIMPACTA_PA:
				nSUM_PA += int(ssystem_buff.sImpacts[nImpact].fADDON_VAR);
				break;
			case EMIMPACTA_SA:
				nSUM_SA += int(ssystem_buff.sImpacts[nImpact].fADDON_VAR);
				break;
			case EMIMPACTA_MA:
				nSUM_MA += int(ssystem_buff.sImpacts[nImpact].fADDON_VAR);
				break;
			case EMIMPACTA_HP_RATE:
				m_fHP_RATE += ssystem_buff.sImpacts[nImpact].fADDON_VAR;
				break;
			case EMIMPACTA_MP_RATE:
				m_fMP_RATE += ssystem_buff.sImpacts[nImpact].fADDON_VAR;
				break;
			case EMIMPACTA_SP_RATE:
				m_fSP_RATE += ssystem_buff.sImpacts[nImpact].fADDON_VAR;
				break;
			case EMIMPACTA_RESIST:
				m_sSUMRESIST_SKILL += int(ssystem_buff.sImpacts[nImpact].fADDON_VAR);
				break;
			};
		}
		...
		for (int nSpec = 0; nSpec < SKILL::MAX_SPEC; ++nSpec)
		{
			...
			{
			case EMSPECA_NONBLOW:
				m_dwHOLDBLOW = ssystem_buff.sSpecs[nSpec].dwSPECFLAG;
				break;
			case EMSPECA_PIERCE:
			...
			case EMSPECA_TARRANGE:
				...
				break;
			case EMSPECA_MOVEVELO:
				m_fSKILL_MOVE += ssystem_buff.sSpecs[nSpec].fSPECVAR1;
				break;

				//	공격 속도 계념이므로 ( 공격 시간을 -0.1 (-10%) 감소 시키기 위해서는 부호 반전을 하여야함. )
			case EMSPECA_ATTACKVELO:
				m_fATTVELO -= ssystem_buff.sSpecs[nSpec].fSPECVAR1;
				break;
			...
			case EMSPECA_DEFENSE_SKILL_ACTIVE:
			{
				...
				m_sDefenseSkill.m_wLevel = static_cast<WORD>(ssystem_buff.sSpecs[nSpec].dwSPECFLAG);
	...
	/*vehicle booster system, Juver, 2017/08/11 */
	if (m_bBoosterStart)
		m_fSKILL_MOVE += (GLCONST_CHAR::fVehicleBoosterAddSpeed / 100.0f);
...
BOOL GLCHARLOGIC_SERVER::CHECKSLOT_ITEM(SNATIVEID sNativeID, EMSLOT emSlot)
{
	...
	if (sItem.sSuitOp.emSuit != emSuit)
		return FALSE;
	...
	if (emSuit == SUIT_HANDHELD)
	{
		...
		else
		{
			...
			{
				if (sItem.sSuitOp.emHand == HAND_RIGHT && emSlot != emRHand)
					return FALSE;
				if (sItem.sSuitOp.emHand == HAND_LEFT && emSlot != emLHand)
					return FALSE;
			}
...
BOOL GLCHARLOGIC_SERVER::ACCEPT_ITEM(SNATIVEID sNativeID)
{
	...
	if (m_wSUM_PA < sItem.sBasicOp.wReqPA)
		return FALSE;
	if (m_wSUM_SA < sItem.sBasicOp.wReqSA)
		return FALSE;

	/*UserNum ItemReq, Juver, 2017/06/27 */
	if (sItem.sBasicOp.dwReqUserNum != 0)
	{
		if (GetUserID() != sItem.sBasicOp.dwReqUserNum)
			return FALSE;
	}
	...
	{
		if (m_dwContributionPoint < sItem.sBasicOp.dwReqContributionPoint)
			return FALSE;
	}
	...
	{
		if (m_dwActivityPoint < sItem.sBasicOp.dwReqActivityPoint)
			return FALSE;
	}
	...
	{
		if (m_llPlayTime < sItem.sBasicOp.llPlayTimeReq)
			return FALSE;
	}
	...
	/*item user requirement, Juver, 2018/05/24 */
	if (!GLItemUser::GetInstance().CharTest(sNativeID.dwID, GetUserID(), m_dwCharID, m_dwGuild))
		return FALSE;
...
BOOL GLCHARLOGIC_SERVER::SIMPLE_CHECK_ITEM(SNATIVEID sNativeID, SNATIVEID sDisguiseID, bool bLevelCheck)
{
	...
	if (!GLItemUser::GetInstance().CharTest(sNativeID.dwID, GetUserID(), m_dwCharID, m_dwGuild))
		return FALSE;
	if (!GLItemUser::GetInstance().CharTest(sDisguiseID.dwID, GetUserID(), m_dwCharID, m_dwGuild))
		return FALSE;
...
WORD GLCHARLOGIC_SERVER::CALC_ACCEPTP(SNATIVEID sNativeID)
{
	...
	if (m_sSUMSTATS.wPow < pITEM->sBasicOp.sReqStats.wPow)
		wATP += pITEM->sBasicOp.sReqStats.wPow - m_sSUMSTATS.wPow;
	if (m_sSUMSTATS.wStr < pITEM->sBasicOp.sReqStats.wStr)
		wATP += pITEM->sBasicOp.sReqStats.wStr - m_sSUMSTATS.wStr;
	if (m_sSUMSTATS.wSpi < pITEM->sBasicOp.sReqStats.wSpi)
		wATP += pITEM->sBasicOp.sReqStats.wSpi - m_sSUMSTATS.wSpi;
	if (m_sSUMSTATS.wDex < pITEM->sBasicOp.sReqStats.wDex)
		wATP += pITEM->sBasicOp.sReqStats.wDex - m_sSUMSTATS.wDex;
	if (m_sSUMSTATS.wInt < pITEM->sBasicOp.sReqStats.wInt)
		wATP += pITEM->sBasicOp.sReqStats.wInt - m_sSUMSTATS.wInt;
	if (m_sSUMSTATS.wSta < pITEM->sBasicOp.sReqStats.wSta)
		wATP += pITEM->sBasicOp.sReqStats.wSta - m_sSUMSTATS.wSta;

	if (m_wLevel < pITEM->sBasicOp.wReqLevelDW)
		wATP += pITEM->sBasicOp.wReqLevelDW - m_wLevel;
...
EMBEGINATTACK_FB GLCHARLOGIC_SERVER::BEGIN_ATTACK(WORD wStrikeNum)
{
	...
	//	SP 여력 채크.
	if (m_sSP.wNow < (wDisSP * wStrikeNum))
		return EMBEGINA_SP;
...
BOOL GLCHARLOGIC_SERVER::ISMASTER_SKILL(SNATIVEID skill_id)
{
	...
	if (DWORD(sSkill.wLevel + 1) >= SKILL::MAX_SKILL_LEVEL)
		return TRUE;
...
EMSKILL_LEARNCHECK GLCHARLOGIC_SERVER::CHECKLEARNABLE_SKILL(SNATIVEID skill_id)
{
	...
	if (learniter != m_ExpSkills.end())
	{
		...
		if (wSKILL_LEVEL == SKILL::MAX_SKILL_LEVEL)
	...
	//	Note : 스킬 습득조건 검사.
	//
	if (!(m_emClass & sSKILL_LEARN.dwCLASS))
	...
	if (WORD(GETLEVEL()) < sSKILL_LEARN_LVL.dwLEVEL)
		return EMSKILL_LEARN_NOTTERM;
	if (!m_sSUMSTATS.CHECK_REQ(sSKILL_LEARN_LVL.sSTATS))
		return EMSKILL_LEARN_NOTTERM;
...
EMSKILLCHECK GLCHARLOGIC_SERVER::CHECHSKILL(SNATIVEID skill_id, WORD wStrikeNum, bool bNotLearn, EMSLOT emRHand, EMSLOT emLHand)
{
	...
	if (m_bProhibitSkill || m_bCaptureTheFlagHoldFlag)
		return EMSKILL_PROHIBIT;
	...
	else
	{
		SKILL_MAP_ITER learniter = m_ExpSkills.find(skill_id.dwID);
		if (learniter == m_ExpSkills.end())
			return EMSKILL_NOTLEARN; // 배운 스킬이 아닐 경우.
	...
	if (emSKILL_LITEM != SKILLATT_NOCARE)
	{
		SITEM *pItem = GET_SLOT_ITEMDATA(emLHand);
		if (!pItem)
			return EMSKILL_NOTITEM;

		if (!CHECHSKILL_ITEM(emSKILL_LITEM, pItem->sSuitOp.emAttack, bHiddenWeapon))
			return EMSKILL_NOTITEM;
	}
	...
	if (emSKILL_RITEM != SKILLATT_NOCARE)
	{
		...
		if (!CHECHSKILL_ITEM(emSKILL_RITEM, pItem->sSuitOp.emAttack, bHiddenWeapon))
			return EMSKILL_NOTITEM;
	}
	...
	//	Note : 기초 체력 소모량 점검.
	if (m_sHP.wNow <= sSKILL_DATA.wUSE_HP * wStrikeNum)
		return EMSKILL_NOTHP;
	if (m_sMP.wNow < sSKILL_DATA.wUSE_MP * wStrikeNum)
	...
	/*combatpoint logic, Juver, 2017/05/28 */
	if (m_sCombatPoint.wNow < sSKILL_DATA.wUSE_CP * wStrikeNum)
	...
	for (int i = 0; i < SLOT_TSIZE; ++i)
	{
		...
		if (pITEM->sBasicOp.sNativeID == sNID)
		{
			if (pITEM->sDrugOp.wPileNum == 1)
				dwNum++;
			else if (pITEM->sDrugOp.wPileNum == sCUSTOM.wTurnNum)
				dwNum++;
		}
	}
...
DWORD GLCHARLOGIC_SERVER::GET_PK_LEVEL()
{
	...
	{
		if (GLCONST_CHAR::sPK_STATE[dwLEVEL].nPKPOINT <= m_nBright)
			break;
	}
...
std::string GLCHARLOGIC_SERVER::GET_PK_NAME()
{
	DWORD dwLEVEL = GET_PK_LEVEL();
	if (dwLEVEL == UINT_MAX)
		return GLCONST_CHAR::sPK_STATE[0].strNAME;
...
DWORD GLCHARLOGIC_SERVER::GET_PK_ITEMDROP_NUM()
{
	...
	if (dwLEVEL == UINT_MAX)
	{
		if (GLCONST_CHAR::fPK_ITEM_DROP == 0)
			return 0;
		return 1;
	}
...
float GLCHARLOGIC_SERVER::GET_PK_ITEMDROP_RATE()
{
	...
	if (GLSchoolFreePK::GetInstance().IsON())
...
{
	if (m_PutOnItems[_slot].sNativeID == NATIVEID_NULL())
		return FALSE;
...
{
	if (m_mapContinuousDamage.empty())
		return FALSE;
	if (_sID == NATIVEID_NULL())
		return FALSE;
...
bool GLCHARLOGIC_SERVER::STATS_RESET()
{
	...
	m_sStats.RESET();
	m_wStatsPoint = static_cast<WORD>(dwCurrentStats);
...
float GLCHARLOGIC_SERVER::get_critical_hit_chance() const
{
	...
	if (val > (int)GLCONST_CHAR::dwCRITICAL_MAX)
		val = GLCONST_CHAR::dwCRITICAL_MAX;
	if (val < 0)
		val = 0;
...
float GLCHARLOGIC_SERVER::get_crushing_blow_chance() const
{
	...
	if (val > (int)GLCONST_CHAR::dwCRUSHING_BLOW_MAX)
		val = GLCONST_CHAR::dwCRUSHING_BLOW_MAX;
	if (val < 0)
		val = 0;
...
EMCHECK_WEAR GLCHARLOGIC_SERVER::CHECK_WEAR_ITEM_SLOT(SNATIVEID sItemID, EMSLOT emPutOnSlot)
{
	...
	if (!pItem)
		return EMCHECK_WEAR_INVALID_ITEM;
			 ...
			 iter_codex_done != m_mapCodexDone.end(); ++iter_codex_done)
		{
			...
			if (pcodex_char)
			{
				if (scodex_char_data.emType == EMCODEX_TYPE_REACH_LEVEL)
					m_dwHPIncreaseDummy += static_cast<WORD>(pcodex_char->dwRewardPoint);
				if (scodex_char_data.emType == EMCODEX_TYPE_KILL_MOB)
					m_dwMPIncreaseDummy += static_cast<WORD>(pcodex_char->dwRewardPoint);
				if (scodex_char_data.emType == EMCODEX_TYPE_KILL_PLAYER)
					m_dwSPIncreaseDummy += static_cast<WORD>(pcodex_char->dwRewardPoint);
				if (scodex_char_data.emType == EMCODEX_TYPE_REACH_MAP)
					m_dwAttackIncreaseDummy += static_cast<WORD>(pcodex_char->dwRewardPoint);
				if (scodex_char_data.emType == EMCODEX_TYPE_TAKE_ITEM)
					m_dwDefenseIncreaseDummy += static_cast<WORD>(pcodex_char->dwRewardPoint);
				if (scodex_char_data.emType == EMCODEX_TYPE_USE_ITEM)
					m_dwShootingIncreaseDummy += static_cast<WORD>(pcodex_char->dwRewardPoint);
				if (scodex_char_data.emType == EMCODEX_TYPE_REACH_CODEX)
					m_dwMeleeIncreaseDummy += static_cast<WORD>(pcodex_char->dwRewardPoint);
				if (scodex_char_data.emType == EMCODEX_TYPE_COMPLETE_QUEST)
					m_dwEnergyIncreaseDummy += static_cast<WORD>(pcodex_char->dwRewardPoint);
				if (scodex_char_data.emType == EMCODEX_TYPE_CODEX_POINT)
					m_dwResistanceIncreaseDummy += static_cast<WORD>(pcodex_char->dwRewardPoint);
				if (scodex_char_data.emType == EMCODEX_TYPE_QUESTION_BOX)
					m_dwHitrateIncreaseDummy += static_cast<WORD>(pcodex_char->dwRewardPoint);
				if (scodex_char_data.emType == EMCODEX_TYPE_ETC)
					m_dwAvoidrateIncreaseDummy += static_cast<WORD>(pcodex_char->dwRewardPoint);
			}
		}
		{
			/* Codex HP, Archie 02/16/24 */
...
{
	m_bRankName = (bShowName != FALSE);
}
void GLCHARLOGIC_SERVER::ShowRankMark(BOOL bShowMark)
{
	m_bRankMark = (bShowMark != FALSE);
}