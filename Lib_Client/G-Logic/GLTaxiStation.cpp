...
HRESULT GLTaxiStation::SaveFile(const char *szFile)
{
	...
	if (!SFile.OpenFile(FOT_WRITE, szFullPath))
		return E_FAIL;
	SFile.SetEncodeType(EMRCRYPT_BYTE_TYPE_2021_TAXI_STATION_V1);
...
{
	if (nIndex < 0 || m_vecTaxiMap.size() <= static_cast<size_t>(nIndex))
		return NULL;
...
STAXI_STATION *GLTaxiStation::GetStation(int nMapIndex, int nStationIndex)
{
	...
	if (nStationIndex < 0 || pTaxiMap->vecTaxiStation.size() <= static_cast<size_t>(nStationIndex))
		return NULL;
