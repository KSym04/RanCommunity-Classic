...
GLCharClient::GLCharClient(void)
	: m_pd3dDevice(NULL), m_pSkinChar(NULL)

	  ,
	  m_vDir(0, 0, -1), m_vDirOrig(0, 0, -1), m_vPos(0, 0, 0), m_vServerPos(0, 0, 0), m_fAge(0.0f), m_Action(GLAT_IDLE), m_dwActState(NULL), m_vTarPos(0, 0, 0), m_fIdleTime(0.0f), m_fMoveDelay(0.0f)

	  ,
	  m_dwANISUBSELECT(0), m_dwANISUBGESTURE(0)

	  ,
	  m_vMaxOrg(6, 20, 6), m_vMinOrg(-6, 0, -6)

	  ,
	  m_vMax(6, 20, 6), m_vMin(-6, 0, -6), m_fHeight(20.f)

	  ,
	  m_CHARINDEX(GLCI_BRAWLER_M), m_dwGaeaID(0), m_dwCeID(0)

	  ,
	  m_pLandManClient(NULL), m_pGlobNode(NULL), m_pQuadNode(NULL), m_pCellNode(NULL)

	  ,
	  m_emANIMAINSKILL(AN_SKILL_A), m_emANISUBSKILL(AN_SUB_NONE)

	  ,
	  m_emANISUBTYPE(AN_SUB_NONE), m_dwRevData(NULL)

	  ,
	  m_fSTATE_MOVE(0.0f), m_fSTATE_DELAY(0.0f)

	  ,
	  m_bSTATE_PANT(false), m_bSTATE_STUN(false)

	  ,
	  m_fSKILL_MOVE(0.0f), m_fITEM_MOVE_R(0.0f), m_fITEM_MOVE(0.0f), m_fATTVELO(0.0f), m_fITEMATTVELO_R(0.0f), m_fITEMATTVELO(0.0f)
	  //, m_dwSummonGUID ( GAEAID_NULL )

	  /*additional RV, Juver, 2018/06/15 */
	  ,
	  fadd_movement_speed(0.0f), fadd_attack_speed(0.0f)

	  /*multi skill Juver, 2018/09/04 */
	  ,
	  m_wTARGET_NUM_MAIN(0), m_wTARGET_NUM_SUB(0), m_vTARGET_POS_MAIN(0, 0, 0), m_vTARGET_POS_SUB(0, 0, 0)

	  ,
	  m_wACTIVESKILL_LEVEL(0)

	  ,
	  m_fattTIMER(0.0f), m_nattSTEP(0), m_bVehicle(FALSE), m_bItemShopOpen(false), m_dwTransfromSkill(SNATIVEID::ID_NULL), m_bSafeZone(false)
	  ...
	  ,
	  m_bSkillTurnInvisible(false), m_wSkillTurnInvisibleLevel(0), m_wSkillTurnInvisibleAnimation(SKILL::EMSPEC_INVISIBLE_ANI_TYPE_NONE), m_bSkillTurnInvisibleDisableInCombat(false), m_sSkillTurnInvisibleID(false), m_bSkillShowInvisible(false), m_wSkillShowInvisibleLevel(0), m_wSkillShowInvisibleRange(0)

	  /* costume combine map restriction, Juver, 2020/09/14 */
	  ,
	  m_bCostumeCombineRestrictMoveSpeed(false), m_bCostumeCombineRestrictAttackSpeed(false)
...
BOOL GLCharClient::IsVisibleDetect()
{
	...
	if (m_bSkillTurnInvisible)
	{
		...
		if (pCharacter)
		{
			if (pCharacter->m_dwUserLvl >= NSUSER_TYPE::USER_TYPE_MASTER)
				return TRUE;
...
HRESULT GLCharClient::Create(GLLandManClient *pLandManClient, SDROP_CHAR *pCharData, LPDIRECT3DDEVICEQ pd3dDevice)
{
	...
	for (int i = 0; i < EMBLOW_MULTI; ++i)
	{
		...
		if (sDropBlow.emBLOW <= EMBLOW_SINGLE)
			...
			pSTATEBLOW = &m_sSTATEBLOWS[sDropBlow.emBLOW - EMBLOW_SINGLE];
	...
	if (pCharData->dwActState & EM_ACT_RUN)
		SetSTATE(EM_ACT_RUN);
	if (pCharData->dwActState & EM_REQ_VISIBLENONE)
		SetSTATE(EM_REQ_VISIBLENONE);
	if (pCharData->dwActState & EM_REQ_VISIBLEOFF)
		SetSTATE(EM_REQ_VISIBLEOFF);
	if (pCharData->dwActState & EM_ACT_PEACEMODE)
...
HRESULT GLCharClient::UpdateSuit()
{
	...
	for (int i = 0; i < SLOT_NSIZE_S_2; i++)
	{
		...
		/*ABL system, Juver, 2017/06/02 */
		if (i == SLOT_LHAND)
			ItemClient = m_CharData.m_PutOnItems[SLOT_RHAND];
		if (i == SLOT_LHAND_S)
			ItemClient = m_CharData.m_PutOnItems[SLOT_RHAND_S];
			...
			if (pItem)
			{
				...
				if (pItem->sBasicOp.emItemType != ITEM_VEHICLE)
				{
					...
					if (i == SLOT_LHAND || i == SLOT_LHAND_S)
						strFileName = pItem->GetWearingFileL(emIndex);
					...
					if (pBoneLink)
						m_pSkinChar->SetAttBone(pBoneLink, m_pd3dDevice, NULL, ItemClient.GETGRADE_EFFECT(), TRUE, strSelfEffect.c_str());
					else
						m_pSkinChar->SetPiece(strFileName.c_str(), m_pd3dDevice, NULL, ItemClient.GETGRADE_EFFECT(), TRUE, strSelfEffect.c_str());
					...
					if (/*!bSetSimple*/ !bCharacterSimple && ItemClient.GETGRADE_EFFECT() >= 6 && (i == SLOT_UPPER || i == SLOT_LOWER || i == SLOT_HAND || i == SLOT_FOOT))
					{
						...
						if (hrSetUpgrade == S_OK)
							bResetUpgradeEffect = FALSE;
					}
...
void GLCharClient::TurnAction(EMACTIONTYPE toAction)
{
	...
	if (m_actorMove.PathIsActive())
	{
		...
		/*dash skill logic, Juver, 2017/06/17 */
		/* skill position shift, Juver, 2020/12/15 */
		if (!IsACTION(GLAT_MOVE) && !IsACTION(GLAT_PUSHPULL) && !IsACTION(GLAT_SKILL_PUSH_PULL) && !IsACTION(GLAT_SKILL_DASH) && !m_bSkillPositionShift)
			m_actorMove.Stop();
	}
}
	...
	case GLAT_MOVE:
	{
		...
		if (m_bVehicle)
		{
			...
			else
			{
				emMType = CHECK_VEHICLE_MAIN(m_sVehicle.m_sVehicleID);
				/*vehicle booster system, Juver, 2017/08/12 */
				emSType = CHECK_VEHICLE_SUB(bVehicleBooster ? AN_ATTACK : IsSTATE(EM_ACT_RUN) ? AN_RUN
																							  : AN_WALK);
			}
		}
		m_pSkinChar->SELECTANI(emMType, emSType);
		...
		/*vehicle booster system, Juver, 2017/08/12 */
		if (m_pSkinChar->m_pVehicle)
			m_pSkinChar->m_pVehicle->SELECTANI(bVehicleBooster ? AN_VEHICLE_BOOSTER : IsSTATE(EM_ACT_RUN) ? AN_VEHICLE_RUN
																										  : AN_VEHICLE_WALK,
											   AN_SUB_NONE);
	}
	break;
...
void GLCharClient::MsgProcess(NET_MSG_GENERIC *nmg)
{
	...
	case NET_MSG_GCTRL_PUSHPULL_BRD:
	{
		...
		if (bSucceed)
		{
			...
			if (pNetMsg->bSkillMove && vMovePos.x != FLT_MAX && vMovePos.y != FLT_MAX && vMovePos.z != FLT_MAX)
			{
				...
				if (pNetMsg->bReverseDir)
					vNewDirection = vMovePos - m_vPos;
	...
	case NET_MSG_GCTRL_VEHICLE_BOOSTER_STATE_START_BRD:
	{
		...
		if (!m_CharData.m_bBoosterStart && IsSTATE(EM_ACT_VEHICLE_BOOSTER))
			ReSetSTATE(EM_ACT_VEHICLE_BOOSTER);
	...
	case NET_MSG_GCTRL_VEHICLE_BOOSTER_STATE_RESET_BRD:
	{
		...
		if (IsSTATE(EM_ACT_VEHICLE_BOOSTER))
			ReSetSTATE(EM_ACT_VEHICLE_BOOSTER);
	...
	if (VALID_SLOT_ITEM(emSlot))
	{
		...
		if (pItem)
		{
			if (pItem->sSuitOp.emAttack <= ITEMATT_NEAR)
			{
				...
				if (strEFFECT.empty())
					strEFFECT = GLCONST_CHAR::GETSTRIKE_EFFECT(emELEMENT);
			...
			else
			{
				...
				{
					SITEM *pLHAND = NULL;
					if (VALID_SLOT_ITEM(SLOT_LHAND))
						pLHAND = GLItemMan::GetInstance().GetItem(GET_SLOT_ITEM(SLOT_LHAND).sNativeID);
...
void GLCharClient::SkillEffect_Main(const SANIMSTRIKE &sStrikeEff)
{
	...
	else if (pSkill->m_sBASIC.emIMPACT_TAR == TAR_SELF_TOSPEC)
	{
		...
		if (!GLGaeaClient::GetInstance().IsVisibleCV(sTarget))
			return;
	...
	else
	{
		...
		if (m_wTARGET_NUM_SUB)
		{
			for (WORD i = 0; i < m_wTARGET_NUM_SUB; ++i)
			{
				...
				if (pskill_sub->m_sEXT_DATA.bTARG_ONE)
					break;
			}
		}
	}
...
void GLCharClient::UPDATE_DATA(float fTime, float fElapsedTime, BOOL bClient)
{
	...
	for (int i = 0; i < SYSTEM_BUFF_SIZE; ++i)
	{
		...
		if (ssystembuff.sNATIVEID == NATIVEID_NULL())
			continue;
	...
	//	Memo :	? 아이템 이벤트 적용
	if (m_CharData.sEVENTFACT.IsACTIVE(EMGM_EVENT_SPEED))
		m_fSKILL_MOVE += (m_CharData.sEVENTFACT.wSpeed / 100.0f);
	if (m_CharData.sEVENTFACT.IsACTIVE(EMGM_EVENT_ASPEED))
		m_fATTVELO += (m_CharData.sEVENTFACT.wASpeed / 100.0f);
...
BOOL GLCharClient::RECEIVE_SKILLFACT(const SNATIVEID skill_id, const WORD wlevel, const DWORD dwSELECT, WORD _wCasterCrow, DWORD _dwCasterID, float fAge)
{
	...
	if (bHOLD)
	{
		...
		if (fAge == _SKILLFACT_DEFAULTTIME)
