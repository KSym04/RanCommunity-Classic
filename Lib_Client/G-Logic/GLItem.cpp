...
{
	if (sBasicOp.IsEVENT())
		return FALSE; // event item
	if (sBasicOp.IsDISGUISE())
		return FALSE; // costume
	if (IsTIMELMT())
		return FALSE; // expiry
	if (sBasicOp.sNativeID.wMainID == 76)
		return FALSE; // gm item
	if (sBasicOp.emItemType != ITEM_SUIT)
		return FALSE; // wear item

	/*wrapper bypass, Juver, 2019/02/20 */
	// this bypasses the conditions below
	if (sBasicOp.bWrapperBypass)
		return TRUE;

	/* block wrapper, Juver, 2021/06/08 */
	// wrapper is blocked
	if (sBasicOp.IsBlockWrapper())
		return FALSE;

	if (sBasicOp.dwReqActivityPoint != 0)
		return FALSE; // activity item
	if (sBasicOp.dwReqContributionPoint != 0)
		return FALSE; // contribution item
...
SITEMCUSTOM::SITEMCUSTOM()
	: sNativeID(false), nidDISGUISE(false), tBORNTIME(0), tDISGUISE(0), lnGenNum(0), wTurnNum(1), cGenType(EMGEN_DEFAULT), cChnID(0)

	  ,
	  cFieldID(0)

	  ,
	  cDAMAGE(0), cDEFENSE(0), cRESIST_FIRE(0)

	  ,
	  cRESIST_ICE(0), cRESIST_ELEC(0), cRESIST_POISON(0), cRESIST_SPIRIT(0)

	  ,
	  cOptTYPE1(0), cOptTYPE2(0), cOptTYPE3(0), cOptTYPE4(0)

	  ,
	  nOptVALUE1(0), nOptVALUE2(0), nOptVALUE3(0), nOptVALUE4(0)

	  ,
	  dwPetID(0), dwVehicleID(0), bVietnamGainItem(false)

	  /*item color, Juver, 2018/01/04 */
	  ,
	  wColor1(ITEMCOLOR_WHITE), wColor2(ITEMCOLOR_WHITE)
...
SITEMCUSTOM::SITEMCUSTOM(SNATIVEID sNID)
	: sNativeID(sNID), nidDISGUISE(false), tBORNTIME(0), tDISGUISE(0), lnGenNum(0), wTurnNum(1)

	  ,
	  cGenType(EMGEN_DEFAULT), cChnID(0)
	  ...
	  ,
	  dwPetID(0), dwVehicleID(0), bVietnamGainItem(0)
...
WORD SITEMCUSTOM::GETGRADE_DAMAGE() const
{
	...
	if (cGRADE > GLCONST_CHAR::wGRADE_MAX)
		cGRADE = static_cast<BYTE>(GLCONST_CHAR::wGRADE_MAX);
	...
	if (cGRADE > GRADE_HIGH)
	{
		...
		//	"고정"
		wDAMAGE_VALUE = static_cast<WORD>(wDAMAGE_VALUE * GLCONST_CHAR::fDAMAGE_GRADE_TOP[cOVER]);
...
WORD SITEMCUSTOM::GETGRADE_DEFENSE() const
{
	...
	if (cGRADE > GRADE_HIGH)
	{
		...
		//	"고정"
		wDEFENSE_VALUE = static_cast<WORD>(wDEFENSE_VALUE * GLCONST_CHAR::fDEFENSE_GRADE_TOP[cOVER]);
...
WORD SITEMCUSTOM::GETGRADE_RESIST_FIRE() const
{
	...
	if (cGRADE > GLCONST_CHAR::wGRADE_MAX)
		cGRADE = static_cast<BYTE>(GLCONST_CHAR::wGRADE_MAX_REGI);
...
{
	if (GETOptTYPE1() == emOPT)
		return GETOptVALUE1();
	else if (GETOptTYPE2() == emOPT)
		return GETOptVALUE2();
	else if (GETOptTYPE3() == emOPT)
		return GETOptVALUE3();
	else if (GETOptTYPE4() == emOPT)
		return GETOptVALUE4();
...
{
	if (GETOptTYPE1() != EMR_OPT_NULL)
		return TRUE;
	else if (GETOptTYPE2() != EMR_OPT_NULL)
		return TRUE;
	else if (GETOptTYPE3() != EMR_OPT_NULL)
		return TRUE;
	else if (GETOptTYPE4() != EMR_OPT_NULL)
		return TRUE;
...
bool SITEMCUSTOM::GENERATE_RANDOM_OPT(bool bNew, BOOL block0, BOOL block1, BOOL block2, BOOL block3)
{
	...
	if (bNew)
	{
		...
		for (int i = 0; i < (int)vecORDER.size() && nGEN < SRANDOM_OPTION_GEN::MAX_OPT && nGEN < sRANDOM.wMaxGenD; ++i)
		{
			...
			if (iter == sRANDOM.mapSET.end())
				continue;

			const SRANDOM_OPTION_SET *pSET = &(*iter).second;

			if (pSET->emTYPE == EMR_OPT_NULL)
				continue;
	...
	else
	{
		...
		for (SRANDOM_OPTION_SET_MAP_CITER iter = sRANDOM.mapSETR.begin(); iter != sRANDOM.mapSETR.end(); ++iter)
		{
			...
			if (block0 && cOptTYPE1_SAVE == opt)
				continue;
			if (block1 && cOptTYPE2_SAVE == opt)
				continue;
			if (block2 && cOptTYPE3_SAVE == opt)
				continue;
			if (block3 && cOptTYPE4_SAVE == opt)
				continue;
		...
		for (int i = 0; i < (int)vecORDER.size() && nGEN < SRANDOM_OPTION_GEN::MAX_OPT && nGEN < sRANDOM.wMaxGenR; ++i)
		{
			...
			if (pSET->emTYPE == EMR_OPT_NULL)
				continue;
			if (pSET->emTYPE == EMR_OPT_GRIND_DAMAGE && cDAMAGE)
				continue;
			if (pSET->emTYPE == EMR_OPT_GRIND_DEFENSE && cDEFENSE)
				continue;
...
WORD SITEMCUSTOM::getmadamage() const
{
	...
	if (fRATE != 0.0f || addvalue != 0.0f)
	{
		...
		if ((int(wMaDAMAGE) + int(addvalue)) < 0)
			wMaDAMAGE = 0;
		else if ((int(wMaDAMAGE) + int(addvalue)) > int(0xffff))
			wMaDAMAGE = 0;
		else
			wMaDAMAGE += (WORD)addvalue;
	}
...
WORD SITEMCUSTOM::get_pa_damage() const
{
	...
	if (addvalue != 0.0f)
	{
		if ((int(value_total) + int(addvalue)) < 0)
			value_total = 0;
		else if ((int(value_total) + int(addvalue)) > int(0xffff))
			value_total = 0;
		else
			value_total += (WORD)addvalue;
	}
...
WORD SITEMCUSTOM::GETREQ_SP() const
{
	...
	if (fVALUE != 0.0f)
	{
		if (fVALUE + wREQSP > 0.0f)
			wREQSP = WORD(wREQSP + fVALUE);
		else
			wREQSP = 0;
	}
...
SITEM_LOBY::SITEM_LOBY()
	: sNativeID(false), nidDISGUISE(false)

	  ,
	  lnGenNum(0), wTurnNum(1), cGenType(EMGEN_DEFAULT), cChnID(0)
...
SITEM_LOBY::SITEM_LOBY(SNATIVEID sNID)
	: sNativeID(sNID), nidDISGUISE(false), lnGenNum(0), wTurnNum(1)
	  ...
	  ,
	  cDAMAGE(0), cDEFENSE(0), cRESIST_FIRE(0), cRESIST_ICE(0), cRESIST_ELEC(0), cRESIST_POISON(0), cRESIST_SPIRIT(0)
...
bool CItemDrop::IsTakeItem(DWORD dwPartyID, DWORD dwGaeaID)
{
	...
	case EMGROUP_PARTY:
		if (dwHoldGID == PARTY_NULL)
			bhold = true;
		else if (dwHoldGID == dwPartyID)
			bhold = true;
		break;
