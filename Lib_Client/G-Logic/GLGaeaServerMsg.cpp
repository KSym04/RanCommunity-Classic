...
void GLGaeaServer::SENDNOTIFICATION_ONMAP(DWORD dwMapID, LPVOID nmg)
{
	...
	{
		if (!pCharNode->Data->m_bRecMsgNotif)
			continue;
		// offline vend
		if (pCharNode->Data->m_bOffVend)
			continue;
...
void GLGaeaServer::SENDTOCLIENT(DWORD dwClientID, LPVOID nmg)
{
	...
	/////////////////////////////////////////////////////////////////////////////
	/*Top Level*/
	GLMSG::SNETPC_UPDATE_EXP *pNetMsg2 = (GLMSG::SNETPC_UPDATE_EXP *)nmg;
	if (pNetMsg2->nmg.nType == NET_MSG_GCTRL_REQ_GLOBAL_RANKING_LEVEL_UPDATE)
	{
		PGLCHAR pChar = GetCharClientID(dwClientID);
		if (pChar)
		{
			GLMSG::SNETPC_REQ_GLOBAL_RANKING_LEVEL_UPDATE NetMsg;
			NetMsg.sTopLevel.dwChaNum = pChar->m_dwCharID;
			StringCchCopy(NetMsg.sTopLevel.szChaName, CHAR_SZNAME, pChar->m_szName);
			NetMsg.sTopLevel.wChaLevel = pChar->m_wLevel;
			NetMsg.sTopLevel.nChaClass = (int)pChar->m_emClass;
			NetMsg.sTopLevel.wChaSchool = pChar->m_wSchool;
			{
				BOOL bGuild = pChar->m_dwGuild ? TRUE : FALSE;
				if (bGuild)
				{
					NetMsg.sTopLevel.wGuNum = (WORD)pChar->m_dwGuild;
					GLClubMan &cClubMan = GLGaeaServer::GetInstance().GetClubMan();
					GLCLUB *pCLUB = cClubMan.GetClub(pChar->m_dwGuild);
					if (pCLUB)
					{
						NetMsg.sTopLevel.wGuMarkVer = (WORD)pCLUB->m_dwMarkVER;
						StringCchCopy(NetMsg.sTopLevel.szGuName, CHAR_SZNAME, pCLUB->m_szName);
					}
				}
			}
			NetMsg.sTopLevel.llnExp = pChar->m_lnExp;
			SENDTOAGENT(&NetMsg);
		}
	}
	return;
...
BOOL GLGaeaServer::RequestCreateInstantMapReq(DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNETREQ_CREATE_INSTANT_MAP_REQ *pNetMsg)
{
	...
	// 이미 생성된 인던에 소유주 이거나 파티 중에 같은 인던을 생성한 사람이 있을경우의 처리
	{
		...
		for (; iter != m_vecInstantMapId.end(); ++iter)
		{
			...
			if (!pLandMan)
				continue;
			if (pLandMan->GetMapID().wMainID != sTargetMapID.wMainID)
				continue;
			if (!pLandMan->IsInstantMap())
				continue;

			if (!pLandMan->IsExcessiveLimitTime())
				continue;
...
BOOL GLGaeaServer::RequestTrade(DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNET_TRADE *pNetMsg)
{
	...
	if (pMapNode1 && pMapNode1->bTradeRestricted)
		return FALSE;

	SMAPNODE *pMapNode2 = GLGaeaServer::GetInstance().FindMapNode(pTarChar->m_sMapID);
	if (pMapNode2 && pMapNode2->bTradeRestricted)
		return FALSE;
...
BOOL GLGaeaServer::RequestTradeMoney(DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNET_TRADE_MONEY *pNetMsg)
{
	...
	if (pMyChar->isPersonalLock(EMPERSONAL_LOCK_INVEN))
		return FALSE;

	//	금액변화.
	if (pNetMsg->lnMoney > pMyChar->m_lnMoney || pNetMsg->lnMoney < 0)
		return FALSE;
...
BOOL GLGaeaServer::RequestTradeItemResist(DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNET_TRADE_ITEM_REGIST *pNetMsg)
{
	...
	if (pITEM->sBasicOp.emItemType == ITEM_PET_CARD)
	{
		...
		if (pMyPet && pMyPet->IsValid() && sITEMCUSTOM.dwPetID == pMyPet->m_dwPetID)
			return S_FALSE;
	}
...
// Trade Success with Name & ItemName, JDevelopments, 7/14/2025
BOOL GLGaeaServer::RequestTradeAgree(DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNET_TRADE_AGREE *pNetMsg)
{
	...
	if (pMyChar->m_sTrade.GetAgree() && pTarChar->m_sTrade.GetAgree())
	{
		...
		do
		{
			...
			// CDebugSet::ToFileWithTime("_trade_logs.txt", "Trade From IGN : [%d][ %s ] Trade to IGN : [%d][ %s ]",
			RLOG::LogFile("_trade_logs.txt", "Trade From IGN : [%d][ %s ] Trade to IGN : [%d][ %s ]",
						  pMyChar->m_dwCharID, pMyChar->m_szName, pTarChar->m_dwCharID, pTarChar->m_szName);

			std::vector<CString> vecMyItemNames;
			std::vector<CString> vecTarItemNames;

			// Items from MyChar to TarChar
			for (int x = 0; x < EMTRADE_INVEN_X; ++x)
			{
				for (int y = 0; y < EMTRADE_INVEN_Y; ++y)
				{
					SINVENITEM *pItem = pMyChar->m_sTrade.GetItem(x, y);
					if (pItem)
					{
						SITEM *pItemData = GLItemMan::GetInstance().GetItem(pItem->sItemCustom.sNativeID);
						if (pItemData)
						{
							vecMyItemNames.push_back(pItemData->GetName());
						}
					}
				}
			}

			// Items from TarChar to MyChar
			for (int x = 0; x < EMTRADE_INVEN_X; ++x)
			{
				for (int y = 0; y < EMTRADE_INVEN_Y; ++y)
				{
					SINVENITEM *pItem = pTarChar->m_sTrade.GetItem(x, y);
					if (pItem)
					{
						SITEM *pItemData = GLItemMan::GetInstance().GetItem(pItem->sItemCustom.sNativeID);
						if (pItemData)
						{
							vecTarItemNames.push_back(pItemData->GetName());
						}
					}
				}
			}

			// MyChar to TarChar announcement
			if (!vecMyItemNames.empty())
			{
				CString strItemList;
				for (size_t i = 0; i < vecMyItemNames.size(); ++i)
				{
					strItemList += vecMyItemNames[i];
					if (i < vecMyItemNames.size() - 1)
						strItemList += ", ";
				}

				CString strAnnounce;
				strAnnounce.Format("[%s] traded [%s] to [%s]",
								   pMyChar->m_szName, strItemList.GetString(), pTarChar->m_szName);

				// CDebugSet::ToFileWithTime("_trade_logs.txt", "%s", strAnnounce.GetString());
				RLOG::LogFile("_trade_logs.txt", "%s", strAnnounce.GetString());

				GLMSG::SNET_SERVER_GENERALCHAT NetAnnounce;
				NetAnnounce.SETTEXT(strAnnounce.GetString());
				NetAnnounce.bNotify = true;
				NetAnnounce.iColor = 6;

				GLGaeaServer::GetInstance().SENDTOALLCLIENT(&NetAnnounce);
			}

			// TarChar to MyChar announcement
			if (!vecTarItemNames.empty())
			{
				CString strItemList;
				for (size_t i = 0; i < vecTarItemNames.size(); ++i)
				{
					strItemList += vecTarItemNames[i];
					if (i < vecTarItemNames.size() - 1)
						strItemList += ", ";
				}

				CString strAnnounce;
				strAnnounce.Format("[%s] traded [%s] to [%s]",
								   pTarChar->m_szName, strItemList.GetString(), pMyChar->m_szName);
			...
			strncpy(NetMsgComplete.szTargetName, pTarChar->m_szName, CHAR_SZNAME);
			NetMsgComplete.szTargetName[CHAR_SZNAME - 1] = '\0';
			SENDTOAGENT(pMyChar->m_dwClientID, &NetMsgComplete);

			strncpy(NetMsgComplete.szTargetName, pMyChar->m_szName, CHAR_SZNAME);
			NetMsgComplete.szTargetName[CHAR_SZNAME - 1] = '\0';
...
BOOL GLGaeaServer::GMKicUser(DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNET_GM_KICK_USER_PROC_FLD *pNetMsgFld)
{
	...
	CDebugSet::ToFileWithTime("gm_cmd_logs.txt", "[%d]%s Kickusercmd [%d]%s", pMyChar->m_dwCharID, pMyChar->m_szName, pTARCHAR->m_dwCharID, pTARCHAR->m_szName);
...
{
	if (!m_bEmulator && dwClientID != m_dwAgentSlot)
		return FALSE;
	...
	else if (pNetMsg->dwFindMob == 3)
		emCrow = CROW_ZONE_NAME;
	else if (pNetMsg->dwFindMob == 4)
		emCrow = CROW_GATE_NAME;
	...
	for (i = 0; i < MAXCROW; i++)
	{
		...
		{
			if (pCrowData->m_sBasic.m_emCrow != emCrow)
				continue;
			if (pCrowData->m_sBasic.m_emCrow != emCrow)
				continue;
		}
...
BOOL GLGaeaServer::GMCtrolMobGenEx(DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNET_GM_MOB_GEN_EX_FLD *pNetMsg)
{
	...
	for (DWORD i = 0; i < wRange; ++i)
		vecRandPosX.push_back(i);
	std::random_shuffle(vecRandPosX.begin(), vecRandPosX.end());
	for (DWORD i = 0; i < wRange; ++i)
		vecRandPosZ.push_back(i);
...
BOOL GLGaeaServer::RequestViewAllPlayer(GLMSG::SNET_GM_VIEWALLPLAYER_FLD_REQ *pNetMsg)
{
	...
	for (; pQuadNode; pQuadNode = pQuadNode->pFindNext)
	{
		...
		for (; pCharCur; pCharCur = pCharCur->pNext)
		{
			...
			if (!pGLChar)
				continue;
			if (pGLChar->m_dwClientID == pCHAR->m_dwClientID)
				continue;
...
BOOL GLGaeaServer::RequestShopInfo(DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNET_GM_SHOP_INFO_REQ *pNetMsg)
{
	...
	for (; iter != m_listSearchShop.end(); ++iter)
	{
		...
		{
			CString strItemInfo;
			if (mapSearchItem.find(pNetMsg->sSearchItemID.dwID) == mapSearchItem.end())
				continue;
...
BOOL GLGaeaServer::RequestConfrontPartyChkMbr(GLMSG::SNETPC_CONFRONTPTY_CHECKMBR2_FLD *pNetMsg)
{
	...
	if (!pCharA || !pCharB)
	{
		...
		if (pCharA)
			GLGaeaServer::GetInstance().SENDTOAGENT(pCharA->m_dwClientID, &NetMsgFB);
		if (pCharB)
			GLGaeaServer::GetInstance().SENDTOAGENT(pCharB->m_dwClientID, &NetMsgFB);
	...
	for (; iter != end; ++iter)
	{
		...
		if (!pMember)
			continue;
		if (pNetMsg->sMapID != pMember->m_sMapID)
			continue;
...
BOOL GLGaeaServer::RequestConfrontPartyEnd(GLMSG::SNETPC_CONFRONTPTY_END2_FLD *pNetMsg)
{
	if (pNetMsg->dwPARTY_A != PARTY_NULL && pNetMsg->dwPARTY_B != PARTY_NULL)
	{
		...
		if (pPartyA && pPartyB)
		{
			...
			if (GLCONST_CHAR::nCONFRONT_WIN_LP != 0 && GLCONST_CHAR::nCONFRONT_LOSS_LP)
			{
				...
				{
					fLIFE_P_A = GLCONST_CHAR::nCONFRONT_WIN_LP + (dwLevelB - dwLevelA) / 2.0f + (sOption.fTAR_HP_RATE - sOption.fHP_RATE);
					LIMIT(fLIFE_P_A, 20.0f, 0.0f);

					fLIFE_P_B = GLCONST_CHAR::nCONFRONT_LOSS_LP - (dwLevelB - dwLevelA) / 2.0f - (sOption.fTAR_HP_RATE - sOption.fHP_RATE);
					LIMIT(fLIFE_P_B, 0.0f, -20.0f);
				}
				else if (pNetMsg->emEND_B == EMCONFRONT_END_PWIN)
				{
					fLIFE_P_B = GLCONST_CHAR::nCONFRONT_WIN_LP + (dwLevelA - dwLevelB) / 2.0f + (sOption.fHP_RATE - sOption.fTAR_HP_RATE);
					LIMIT(fLIFE_P_B, 20.0f, 0.0f);

					fLIFE_P_A = GLCONST_CHAR::nCONFRONT_LOSS_LP - (dwLevelA - dwLevelB) / 2.0f - (sOption.fHP_RATE - sOption.fTAR_HP_RATE);
	...
	if (pNetMsg->dwPARTY_A != PARTY_NULL)
	{
		...
		if (pParty)
			pParty->RESET_CONFRONT(pNetMsg->emEND_A);
	}
	...
	if (pNetMsg->dwPARTY_B != PARTY_NULL)
	{
		...
		if (pParty)
			pParty->RESET_CONFRONT(pNetMsg->emEND_B);
	}
...
BOOL GLGaeaServer::ServerClubStorageGetDB(GLMSG::SNET_CLUB_STORAGE_GET_DB *pNetMsg)
{
	...
	if (pCHAR->m_dwGuild != pCLUB->m_dwID)
		goto _END;
	if (pCHAR->m_dwCharID != pCLUB->m_dwMasterID)
		goto _END;
...
BOOL GLGaeaServer::ServerClubAllianceDis(GLMSG::SNET_CLUB_ALLIANCE_DIS_FLD *pNetMsg)
{
	...
	for (; pos != end; ++pos)
	{
		...
		if (sALLIANCE.m_dwID == pCLUB_CHIEF->m_dwID)
...
BOOL GLGaeaServer::RequestChargedItem2Inven(DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNET_CHARGED_ITEM2_INVEN *pNetMsg)
{
	...
	if (pNetMsg->dwUserID != pChar->GetUserID())
		goto _DB_RESTORE;
...
BOOL GLGaeaServer::RequestConftSPtyExp(GLMSG::SNET_CONFT_SPTY_EXP_FLD *pNetMsg)
{
	...
	for (; pCharNode; pCharNode = pCharNode->pNext)
	{
		...
		if (pChar->m_wSchool != pNetMsg->wSCHOOL)
			continue;
...
BOOL GLGaeaServer::RequestInvenTeleport(DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNETPC_REQ_INVEN_TELEPORT *pNetMsg)
{
	...
	if (pChar->CheckCoolTime(pINVENITEM->sItemCustom.sNativeID))
		return FALSE;
...
BOOL GLGaeaServer::RequestBus(DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNETPC_REQ_BUS *pNetMsg)
{
	...
	PGLCROW pCROW(NULL);
	if (pChar->m_pLandMan)
		pCROW = pChar->m_pLandMan->GetCrow(pNetMsg->dwNPC_ID);
	if (!pCROW)
	{
		NetMsgFB.emFB = EMBUS_TAKE_FAIL;
		SENDTOCLIENT(dwClientID, &NetMsgFB);
...
BOOL GLGaeaServer::RequestClubAdd2Fld(GLMSG::SNET_CLUB_MEMBER_ADD_2FLD *pNetMsg)
{
	...
	{
		const GLCLUBMEMBER &cMEMBER = (*pos).second;
		if (cMEMBER.dwID == pNetMsg->dwMember)
			continue; //	자기 자신의 경우 제외.

		pMember = GetCharID(cMEMBER.dwID);
		if (!pMember)
			continue; //	멤버원이 존제 할 경우만.
...
BOOL GLGaeaServer::RequestClubSubMaster(GLMSG::SNET_CLUB_SUBMASTER_FLD *pNetMsg)
{
	...
	if (pSUB)
	{
		...
		if (pCLUB->IsMemberFlgCDCertify(dwCHARID))
...
BOOL GLGaeaServer::RequestClubAuthority(GLMSG::SNET_CLUB_AUTHORITY_FLD *pNetMsg)
{
	...
	if (pNetMsg->dwCharID == GAEAID_NULL)
		return FALSE;
...
BOOL GLGaeaServer::msg_character_disconnect(DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNET_CHARACTER_DISCONNECT_AGENT_TO_FIELD *pNetMsgFld)
{
	...
	CONSOLEMSG_WRITE("[SECURITY ALERT] DISCONNECT ATTACK BLOCKED!");
	CONSOLEMSG_WRITE("  Client ID: %u | Target Player: %s [ID:%u]",
	...
	CDebugSet::ToFileWithTime("_security_disconnect_attacks.txt",
							  "BLOCKED FIELD DISCONNECT ATTACK - Client: %u | Target: %s [%u]",
							  dwClientID, pchar->m_szName, pchar->m_dwCharID);

	// ABSOLUTE PROTECTION: Never execute disconnect under any circumstances
	// ReserveDropOutPC ( pchar->GetCtrlID() );  // PERMANENTLY DISABLED

	// Log the original behavior for debugging
	if (RPARAM::bCharacterDcLogs == 1)
	{
		CDebugSet::ToFileWithTime("_character_disconnect.txt",
								  "BLOCKED field disconnect [%u] %s", pchar->m_dwCharID, pchar->m_szName);
	}
