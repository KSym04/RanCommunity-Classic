		...
		case EMCONFT_GUILD:
		{
			...
			if (pMY_CLUB)
			{
				...
				for (; pos != end; ++pos)
				{
					...
					if (!pTAR)
						continue;
					if (!pTAR->m_sCONFTING.IsCONFRONTING())
						continue;
...
inline HRESULT GLChar::MsgAttack(NET_MSG_GENERIC *nmg)
{
	if (!IsValidBody())
		return E_FAIL;
	if (m_pLandMan && m_pLandMan->IsPeaceZone())
		return E_FAIL;
	...
	if (ISLONGRANGE_ARMS())
		wAttackRange += (WORD)GETSUM_TARRANGE();
...
BOOL GLChar::CheckStartMap()
{
	...
	if (pLandMan)
	{
		if (pLandMan->m_bClubDeathMatchMap)
		{
			...
			if (pCDM && !pCDM->IsEnterMap(m_dwGuild))
				return FALSE;
			...
			if (pClub->GetAllBattleNum() > 0)
				return FALSE;
			if (!pClub->IsMemberFlgCDM(m_dwCharID))
				return FALSE;
		}
...
HRESULT GLChar::MsgReqRecovery(NET_MSG_GENERIC *nmg)
{
	...
	if (m_nDECEXP > m_sExperience.lnNow)
		m_nDECEXP = m_sExperience.lnNow;
...
HRESULT GLChar::MsgReqGetExpRecovery(NET_MSG_GENERIC *nmg)
{
	...
	nDecExp = m_nDECEXP;
	if (nDecExp > m_sExperience.lnNow)
		nDecExp = m_sExperience.lnNow;
...
HRESULT GLChar::MsgActState(NET_MSG_GENERIC *nmg)
{
	...
	if (m_dwUserLvl >= NSUSER_TYPE::USER_TYPE_MASTER || GLCONST_CHAR::bBATTLEROYAL)
	{
		if (pNetMsg->dwActState & EM_REQ_VISIBLENONE)
			SetSTATE(EM_REQ_VISIBLENONE);
		else
...
HRESULT GLChar::MsgReqNpcItemTrade(NET_MSG_GENERIC *nmg)
{
	...
	if (pTalk->m_sNeedItem[0].dwItemID != pNetMsg->dwA_NID[0] ||
		...
		pTalk->m_dwACTION_PARAM2 != pNetMsg->dwB_NID)
		goto _REQ_FAIL;
...
HRESULT GLChar::MsgVehicleSlotExHold(NET_MSG_GENERIC *nmg)
{
	...
	{
		// 탈것용 아이템이 아닐때 / Security: OOB suit type ? potential CRIT-03 exploit
		GLGaeaServer::GetInstance().LogSecurityEvent(m_dwClientID, "CRIT-03: MsgVehicleSlotExHold ? out-of-bounds emSuit value");
	...
	if (!m_sVehicle.CheckSlotItem(pHoldItem->sBasicOp.sNativeID, pNetMsg->emSuit))
		return E_FAIL;
...
HRESULT GLChar::MsgVehicleHoldToSlot(NET_MSG_GENERIC *nmg)
{
	...
	{
		// 탈것용 아이템이 아닐때 / Security: OOB suit type ? potential CRIT-03 exploit
		GLGaeaServer::GetInstance().LogSecurityEvent(m_dwClientID, "CRIT-03: MsgVehicleHoldToSlot ? out-of-bounds emSuit value");
		NetMsgFB.emFB = EMVEHICLE_REQ_SLOT_EX_HOLD_FB_INVALIDITEM;
		GLGaeaServer::GetInstance().SENDTOCLIENT(m_dwClientID, &NetMsgFB);
		return E_FAIL;
	}
...
HRESULT GLChar::MsgVehicleGiveBattery(NET_MSG_GENERIC *nmg)
{
	...
	// Security: prevent battery duplication via rapid packet flooding
	if (m_bBatteryPending)
		return E_FAIL;
	...
	case ITEM_DRUG_HP:
	{
		...
		// Security: set pending flag to prevent duplicate battery use via rapid packets
		m_bBatteryPending = true;
...
{
	// Security: clear battery pending flag so player can use batteries again
	m_bBatteryPending = false;
	...
	{
		// Security: bounds check emType before array access
		if (pIntMsg->emType >= VEHICLE_TYPE_SIZE)
			break;
		if (!GLCONST_VEHICLE::pGLVEHICLE[pIntMsg->emType])
			break;
...
HRESULT GLChar::MsgProcess(NET_MSG_GENERIC *nmg)
{
	...
	case NET_MSG_GCTRL_PMARKET_REGITEM:
		MsgPMarketReqItem(nmg);
		break;
	case NET_MSG_GCTRL_PMARKET_DISITEM:
		MsgPMarketDisItem(nmg);
		break;
	...
	case NET_MSG_PET_GETRIGHTOFITEM_A:
		MsgReqGetRightOfItem_A(nmg);
		break;
	case NET_MSG_PET_GETRIGHTOFITEM_B:
		MsgReqGetRightOfItem_B(nmg);
		break;
	...
	case NET_MSG_REQ_GATHERING_CANCEL:
		MsgGatheringCancel(nmg);
		break;

	// itemmall
	case NET_MSG_RETRIEVE_POINTS:
		MsgRetrievePoints(nmg);
		break;
	...
	case NET_MSG_GCTRL_INVEN_CONSUME_FOOD:
		MsgReqConsumeFood(nmg);
		break;
	case NET_MSG_GCTRL_INVEN_UNLOCK_FOOD:
		MsgReqUnlockFood(nmg);
		break;
...
inline HRESULT GLChar::MsgReqStatsUpModern(NET_MSG_GENERIC *nmg)
{
	...
	if (m_wStatsPoint == 0)
		return S_FALSE;
	if (pNetMsg->sStats.GetTotal() > m_wStatsPoint)
		return S_FALSE;
	if (pNetMsg->sStats.GetTotal() == 0)
		return S_FALSE;
	if (pNetMsg->sStats.isOverFlow())
		return S_FALSE;
...
HRESULT GLChar::MsgItemShopOpen(NET_MSG_GENERIC *nmg)
{
	...
	if (m_bItemShopOpen == NetMsg->bOpen)
		return E_FAIL;
