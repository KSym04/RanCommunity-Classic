...
WORD GLSUMMONLOGIC_SERVER::GETSKILLRANGE_APPLY(const GLSKILL &sSKILL, const WORD dwLEVEL) const
{
	...
	if (sSKILL.m_sBASIC.emAPPLY == SKILL::EMAPPLY_PHY_LONG)
		dwRANGE += (WORD)GETSUM_TARRANGE();

	/*skill range spec logic, Juver, 2017/06/06 */
	if (sSKILL.m_sBASIC.emIMPACT_SIDE == SIDE_ENEMY)
		dwRANGE += (WORD)m_fSUM_SKILL_APPLYRANGE;
...
EMSKILLCHECK GLSUMMONLOGIC_SERVER::CHECHSKILL(DWORD dwAType, bool bNotLearn)
{
	...
	if (GETHP() <= (static_cast<DWORD>(sSKILL_DATA.wUSE_HP) * static_cast<DWORD>(wStrikeNum)))
		return EMSKILL_NOTHP;
	if (GETMP() < static_cast<int>(sSKILL_DATA.wUSE_MP) * static_cast<int>(wStrikeNum))
									   ...
									   DWORD dwWeatherFlag /* =NULL */, DWORD dwDivCount /* =0 */)
{
	...
	if (nPercentCri > static_cast<int>(GLCONST_CHAR::dwCRITICAL_MAX))
		nPercentCri = static_cast<int>(GLCONST_CHAR::dwCRITICAL_MAX);
	...
	if (nNetDAMAGE > 0)
		...
		rResultDAMAGE = int(nNetDAMAGE + (nDAMAGE_OLD * GLCONST_CHAR::fLOW_SEED_DAMAGE) * RANDOM_POS);
	...
	if (bCritical)
		rResultDAMAGE = int(rResultDAMAGE * GLCONST_CHAR::dwCRITICAL_DAMAGE * 0.01f);
...
void GLSUMMONLOGIC_SERVER::UPDATE_DATA(float fTime, float fElapsedTime, BOOL bClient)
{
	...
	for (int i = 0; i < SKILLFACT_SIZE; ++i)
	{
		...
		for (int nImpact = 0; nImpact < static_cast<int>(SKILL::MAX_IMPACT); ++nImpact)
		{
		...
		for (int nSpec = 0; nSpec < static_cast<int>(SKILL::MAX_SPEC); ++nSpec)
		{
