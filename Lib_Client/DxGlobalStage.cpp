...
{
	if (!m_bEmulate)
		m_NetClient.Send((char *)nmg, nmg->dwSize);
	else
		_insert_emul_msg(nmg);
}

void DxGlobalStage::NetSendToField(NET_MSG_GENERIC *nmg)
{
	if (!m_bEmulate)
		m_NetClient.SendToField((char *)nmg, nmg->dwSize);
	else
		_insert_emul_msg(nmg);
}
...
DxGlobalStage::DxGlobalStage(void) : m_hWnd(NULL),
									 m_dwState(NULL),
									 m_bEmulate(FALSE),
...
HRESULT DxGlobalStage::OneTimeSceneInit(const char *szAppPath, HWND hWnd, BOOL bEmulate, WORD wWidth, WORD wHeight, const char *szMapList)
{
	...
	if (!szMapList)
		GLGaeaClient::GetInstance().LoadMapsListFile("mapslist.mst");
	else
		GLGaeaClient::GetInstance().LoadMapsListFile(szMapList);
...
{
	if (m_emThisStage == emChangeStage)
		return S_FALSE;
...
void DxGlobalStage::MsgProcessFrame(NET_MSG_GENERIC *nmg)
{
	...
	switch (nmg->nType)
	{
	case NET_MSG_CLIENT_CHECK:
	{
		NET_CLIENT_CHECK *pNetMsg = (NET_CLIENT_CHECK *)nmg;
		pNetMsg->SOLVE();

		NET_CLIENT_CHECK_FB msg;
		msg.nResult = pNetMsg->nResult;

		switch (pNetMsg->nServer)
		{
		case NET_STATE_AGENT:
			NETSEND(&msg);
			break;
		case NET_STATE_FIELD:
			NETSENDTOFIELD(&msg);
			break;
		}
	}
	...
	case NET_MSG_GCTRL_REQ_GLOBAL_RANKING_LEVEL_AGT: /*Top Level*/
	...
	{
		// SECURITY FIX: Disable mass disconnect functionality
		// This was causing unwanted player disconnections during SERVER_INFO updates
		GLMSG::SNET_CHARACTER_DISCONNECT_AGENT_TO_CHAR *pnetmsg = (GLMSG::SNET_CHARACTER_DISCONNECT_AGENT_TO_CHAR *)nmg;

		// CLIENT NOTICES DISABLED - Only server-side logging remains active
		// Simple logging without complex IP tracking to ensure stability
		CDebugSet::ToFileWithTime("_security_disconnect_attacks.txt",
								  "BLOCKED DISCONNECT ATTACK - CharID: %u | ForceClose: %s",
								  pnetmsg->char_id, pnetmsg->bCloseClient ? "true" : "false");

		// ABSOLUTE PROTECTION: Never execute these under any circumstances
		// GetNetClient()->CloseConnect();  // PERMANENTLY DISABLED
		// CloseGame();                     // PERMANENTLY DISABLED

		// Simple connection maintenance without complex calls
		// Just ensure we don't execute the dangerous functions above
	}
	break;
...
{
	if (m_emThisStage != EM_STAGE_GAME)
		return;
